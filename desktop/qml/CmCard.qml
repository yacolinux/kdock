...
Item {
    ...
    // -1 = inherit the general widget opacity; 0..100 = this card's own override.
    required property int cardOpacity
    ...
    // Background opacity applied to the card's fill only (not its text/content):
    // this card's own value if set, otherwise the general one from the config.
    // 1.0 (the default general value) reproduces the historic look exactly.
    readonly property real effectiveOpacity: cardOpacity >= 0
        ? cardOpacity / 100
        : cmConfig.widgetOpacity
    ...
    Rectangle {
        ...
        anchors.fill: parent
        radius: 10
        // The card's fill, faded by the effective opacity. The border and the
        // content below keep their own opacity so a see-through card stays
        // readable.
        color: Qt.rgba(card.cardColor.r, card.cardColor.g, card.cardColor.b,
                       card.cardColor.a * card.effectiveOpacity)
        clip: true
