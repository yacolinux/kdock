...
CmCardRecord cardFromJson(const QJsonObject &o)
{
    ...
    r.opacity = o.contains(QStringLiteral("op"))
                    ? qBound(0, o.value(QStringLiteral("op")).toInt(), 100) : -1;
    return r;
}
...
void CmLayout::setCardProperty(const QString &id, const QString &key, const QVariant &value)
{
    ...
    if (key == QLatin1String("bg"))
        ...
        // null or a negative number = back to inherit; otherwise a percentage.
        r.opacity = (value.isNull() || value.toInt() < 0) ? -1
                                                          : qBound(0, value.toInt(), 100);
    else
        return;
