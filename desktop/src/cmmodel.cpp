...
QVariant CmModel::data(const QModelIndex &index, int role) const
{
    ...
    case OpacityRole:
        // Raw per-card value: -1 = inherit the general widget opacity, which
        // CmCard.qml resolves against cmConfig.widgetOpacity (same split as the
        // background role, so the card menu can tell "own" from "inherit").
        return c.opacity;
    }
    return {};
}
