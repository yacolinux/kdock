...
int main(int argc, char *argv[])
{
    ...
    app.setApplicationName(QStringLiteral("kdock-desktop"));
    app.setOrganizationName(QStringLiteral("kdock"));
    app.setApplicationDisplayName(QStringLiteral("kdock Desktop Widgets"));
    app.setDesktopFileName(QStringLiteral("kdock-desktop"));
    // The canvas is always on screen; even so, the settings dialog can be the
    // only visible window for a while, so never quit on last-window-closed.
