...
- **Hotplug**: `sync()` runs on `screenAdded`/`screenRemoved`/`primaryScreenChanged` and on enable/disable. It creates a dock only when its dockId is enabled, its screen is connected, and `wantedDocks(currentDesktop())` selects it; it destroys every other live instance. A change of **primary** recreates the affected instances because only the primary dock hosts relanzadores.
...
- `kdock-clipboard` owns the session-bus name `org.kdock.Clipboard`, is started/toggled asynchronously by the launcher, and keeps its own `ClipboardHistory` instance. It captures through `ext-data-control-v1` (with QClipboard fallback), persists the existing JSON/PNG/text history, and runs independently from the dock.
