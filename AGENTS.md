...
src/
  ...
  clipboardlauncher.{h,cpp} — D-Bus launcher for the isolated kdock-clipboard process
  ...
  systraylauncher.{h,cpp} — kdock's whole side of the system tray: find the kdock-systray
                            binary, toggle its window over D-Bus, preload it (token systray).
                            The SNI host/watcher, model, image provider and DBusMenu client
                            moved OUT of src/ into systray/src/ (their own binary) — see
                            *System tray* below.
...
clipboard/
  kdock-clipboard         — Standalone clipboard history application; kdock only launches it
protocols/
...
### Widget "Apps Seleccionables" (tokens `appsel1`, `appsel2`…, 2026-08-13)

El bloque de apps como **sección repetible**: mismo `appsComp`, mismo delegate, mismo menú de
clic derecho, pero con su propia lista de lanzadores. Es el único widget del que un dock puede
tener **más de uno**, y para eso cada instancia tiene su token numerado.

- **Identidad**: `DockConfig::isAppsWidgetToken()` (`^appsel[1-9][0-9]*$`). Los tokens entran en
  `isRepeatableToken()`, que es lo que hace dos cosas imprescindibles: `reconcileWidgetOrder()`
  no los descarta (se inventan en tiempo de ejecución, así que no están en
  `knownWidgetTokens()`) y `removeSectionAt()` los puede borrar. `insertAppsWidget(at)` elige el
...
- **Modelo**: un `DockModel` por instancia, con el token como quinto parámetro del constructor.
  Lo crea y cachea `DockWindow::appsModelFor(token)` (Q_INVOKABLE), y el componente lo pide por
  la property `sectionToken` que el delegate de la sección le empuja con un `Binding` — el
  `Component` está declarado en la raíz de `Dock.qml` y **no ve** el `sec` del delegate. Con
  token: los lanzadores salen de `widgetApps()` y se guardan ahí (`pinnedIds()`/`savePinnedIds()`,
  por donde pasan `togglePinned` y `moveItem`, así que anclar en un widget nunca toca el `pinned`
...
- **Bloque de sobrantes** (`excludeOthers`, 2026-08-13): con el filtro prendido el widget **no le
  ...
  del modelo). Con `onlyPinned` apagado eso es exactamente "Mostrar apps sin repetir lo que ya se
  ve": el widget recoge todo lo abierto que ningún otro dibuja. Dos decisiones:
  - **Nunca esconde sus propios lanzadores.** El filtro está en `acceptsStrayWindow()`, que solo
    se pregunta por una ventana que todavía no tiene fila; una app que el usuario le agregó a
    mano a *este* widget se dibuja igual, aunque esté también en la lista de otro. Esconderla
    haría desaparecer un ícono agregado a propósito, sin explicación.
  - **La lista de los otros es una entrada de este modelo**, así que se reconstruye con
    `widgetAppsChanged` de **cualquier** token (no solo el propio), con
    `widgetExcludeOthersChanged` y con `widgetOrderChanged` (agregar o quitar un widget cambia
...
- **Desagregar ventanas** (`ungroupWindows`, default false, 2026-08-14): un ícono **por ventana**
  en vez de uno por app, solo para este widget — dos ventanas del navegador o dos konsole dibujan
  dos íconos. Los cinco puntos que importan:
  - **Es un estrechamiento de la casilla del dock, no una copia**: `DockModel::groupsWindows()` es
    `groupWindows && !widgetUngroupWindows(token)`, y **reemplazó a las dos lecturas directas** de
    `m_config->groupWindows()` que había en el modelo (`keyForWindow()` y `windowChanged()`). Un
    widget solo puede desagregar **de más**: con la casilla del dock apagada ya está todo
    desagregado, y por eso la del widget se grisa (con un tooltip que lo dice) en vez de mentir.
  - **La segunda ventana de una app anclada NO es una ventana suelta.** El mecanismo de siempre
    (la primera ventana se sienta en la fila del lanzador, las demás se keyean por puntero) las
    manda a `acceptsStrayWindow()`, donde *Ver solo anclados* —prendido por defecto en estos
    widgets— las descartaba: la feature no se veía. La exención (`listsApp()`, la lista propia del
    widget) va **antes** de esa casilla y de los dos filtros de exclusión, igual que la regla de
    que un filtro nunca esconde los lanzadores propios.
  - **Los íconos de más van al final** de la lista del widget (los lanzadores primero), que es lo
    que ya hacía el bloque de apps del dock en modo desagregado.
  - **Todas las filas de una app anclada se leen como ancladas** (`PinnedRole` =
    `item.pinned || pinnedByApp(item)`). Sin eso, el clic derecho de la ventana extra ofrecía
    *Anclar* sobre una app que **ya está en la lista**: aceptarlo dejaba dos filas ancladas detrás
    de una sola entrada, y la de más sobrevivía a su ventana como lanzador huérfano que relanza en
    cada clic. Ahora ofrece *Desanclar*, y `togglePinned()` saca la app de la lista y limpia la
    bandera en **la fila que la lleva** (la del lanzador), esté clickeada o no; el `emit
    dataChanged` va sobre el modelo entero, porque cambian varias filas y no solo una.
  - **Nombre e ícono no cambian**: los dos íconos dicen lo mismo (el título de la ventana ya está
    en el tooltip, `delegateRoot.title || name`). Poner el título como etiqueta haría que el ancho
    del dock se recalculara con cada cambio de título — `labelStrings()` alimenta la medición.
  Lo cubren cinco casos de `tests/unit/tst_appswidget.cpp` (con `FakeWindow`, sin compositor); el
  control positivo es hacer que `groupsWindows()` ignore la bandera: cuatro de los cinco fallan.
- **El widget que desancla se relee a sí mismo** (2026-08-15, reportado como *"desanclo y no
  aparece en el otro widget"*). `savePinnedIds()` levanta `m_updatingPinned` para que el modelo
  no se reconstruya por su propia escritura —correcto en el bloque de apps, donde una app
  desanclada que sigue abierta conserva su ícono—, pero un appsel **filtra** por esa lista: con
  `onlyPinned` la fila que acaba de sacar tiene que irse. Sin eso el widget seguía dibujando una
  app que ya no lista hasta que algo ajeno lo reconstruyera, y desde afuera se ve igual que si el
  desanclado no hubiera pasado. Por eso `togglePinned()` termina con un `rebuild()` **solo si es
  un widget** (`m_widgetToken` no vacío). Medido con la config real del usuario: antes, el widget
  quedaba con 5 filas listando 3 apps; después, 3 — y las ventanas aparecen en los dos bloques de
  sobrantes del monitor, que es lo que se esperaba.
- **Recarga manual** (`reloadAppsWidget(token)`, 2026-08-15): `syncAll()` (vuelca y **relee** el
  `.conf` de cada dock vivo) + `widgetAppsChanged(token)` para el modelo propio +
  `notifyMonitorAppsChanged()` para los de los otros docks del monitor. Es la salida para lo que
  ninguna señal cubre —una lista cambiada a espaldas del proceso— y es manual **a propósito**: la
  alternativa es un timer. Dos puertas, las dos llamando a esto: el ítem *Recargar este widget*
  del submenú nuevo y el botón *Recargar* del panel del diálogo, a la derecha de *Quitar*.
- **Submenú del clic derecho** (`qml/AppsWidgetMenu.qml`, 2026-08-15): las cuatro casillas de la
  instancia más la recarga, colgado del **menú del ícono** (`contextMenu` de `appsComp`), que es
  el único clic derecho que un appsel tiene — es un bloque, así que su `MouseArea` de sección
  está apagada y no hay menú de sección. Tres cosas que no son obvias:
  - **La fila de un submenú no se esconde con `visible`**: en un `Menu` esa property es la del
    popup, y la fila la construye el `delegate` del menú padre. Va por una property opcional
    `rowVisible` que lee `SubMenuDelegate` (default true, o sea que ningún otro submenú cambia),
    y el appsel la ata a `sectionToken !== ""` para que el bloque `apps` del dock no la vea.
  - **Los getters son `Q_INVOKABLE` sin NOTIFY**, así que un `checked:` pelado se congela con lo
    que había al construir el menú. Mismo truco que `gapRevision`: un `rev` local que bumpean
    cuatro `Connections` filtrando por token.
  - **La regla del superconjunto se mudó al setter**: `setWidgetExcludeMonitor(true)` apaga
    `excludeOthers`. Estaba en el diálogo, y ahora las mismas dos casillas están en el menú:
    política escrita en una sola de las dos UI es política que se separa. Por lo mismo el panel
    del diálogo escucha las cuatro señales `*Changed(token)` (dirección config → casilla, que
    antes no tenía).
...
- **UI**: *Diseño → "Agregar Apps Seleccionables"* lo inserta (y *Quitar separador* lo saca:
  ...
  sin renombrar"). Sus apps y sus cuatro casillas están en **Widgets → Apps
  Seleccionables**, un panel por instancia (`rebuildAppsWidgetsGroup()`), que se reconstruye al
  agregar o quitar uno porque cambiar de solapa no reconstruye el diálogo. *Desagregar ventanas*
...
- **Un interruptor por bloque, y son independientes.** El mismo `appsComp` dibuja el bloque de
  ...
  resuelve cuál manda: token vacío → `config.appPreview` (compartida, solapa General); token
  `appsel<n>` → `config.widgetAppPreview(token)` (por widget). **Ninguno es maestro del otro**:
  previews en un solo widget y en ningún otro lado es una configuración válida, así que no hay
  casilla deshabilitada ni dependencia escondida. Lo que **sí** es común es el tamaño y el
  retardo (`appPreviewSize`/`appPreviewDelayMs`, una sola vez en General).
  Las dos arrancan en **false**.
- **Eso es también toda la historia de performance**: nada acá es periódico, y el interruptor
  se lee *antes* que nada en `queueAppPreview()`, así que un bloque apagado no le pregunta ni
  una fila al modelo, mucho menos una captura a KWin. Solo trabajan los bloques habilitados.
- **La casilla del widget está en dos lados**: el panel de la solapa Widgets
  (`createAppsWidgetPanel()`) y el submenú de clic derecho del propio widget
  (`qml/AppsWidgetMenu.qml`), como las otras cuatro. Las dos escriben la misma clave y se
...
- **Qué ventana**: `DockModel::previewWindow(row)` devuelve
  ...
  El `width`/`height` es la geometría de la ventana y da la **proporción** de la micro ventana
  antes de que llegue la captura, así que la superficie nunca se redimensiona bajo el puntero.
- **De dónde salen los píxeles**: `ScreenShotSource` + `ThumbnailCache` +
  `ThumbnailImageProvider`, los tres **mudados de `previews/src/` a `src/`** y compilados en
  los dos binarios. `AppPreviews` los envuelve: uno por proceso (viaja en `DockManager::Shared`),
  reusa una captura de menos de 800 ms, tira la miniatura cuando la ventana se cierra
  (`WindowMonitor::windowRemoved`) y desaloja por LRU pasadas 12. Inerte hasta que QML pide:
  con la feature apagada no hay ni una llamada a D-Bus.
- **El uuid ahora vive en `AbstractWindow`** (`src/windowmonitor.h`), puesto por
  `PlasmaWindowMonitor` con el que ya traía `window_with_uuid`. Sin señal: no dispara ningún
  `rebuild()` de `DockModel`.
- **Por qué la micro ventana es un `Window` con `Qt.ToolTip`** y no un `ToolTip`/`Popup` de
  QQC2: con transient parent, la integración xdg-shell de Qt lo convierte en un **xdg_popup sin
  grab**, que es lo que `LayerSurface::attachPopup()` cuelga de la superficie layer-shell — la
  misma máquina de los menús del dock. De ahí salen las dos mitades del requerimiento: un
  xdg_popup **no es un toplevel**, así que `plasma-window-management` no lo reporta y no puede
...
- **Nunca se muestra sin captura**: si no hay revisión todavía, la ventana queda oculta hasta
  que llega (`Connections` sobre `appPreviews.thumbnailReady`). O sea que un ScreenShot2 sin
  autorizar degrada a *"no aparece nada"* en vez de a un marco vacío. La captura se toma **una
  sola vez, al abrir el preview** (`showAppPreview → appPreviews.request`): es una foto del
  momento en que el puntero aterrizó y **no** se refresca en un timer mientras sigue arriba
  (había un timer de 1 s que re-pedía la captura; se sacó el 2026-08-26 — un re-render por
  segundo en KWin bajo un cursor quieto no lo pedía nadie). El estado que sí debe seguir vivo
  (los botones minimizar/maximizar) lo relee `refreshPreviewState()` del modelo, que es una
  lectura de propiedades barata, no una captura.
- **El tooltip de texto del ícono se apaga mientras el preview está arriba** (`&& !appPreview.visible`).
- **Autorización**: `kdock.desktop.in` ahora declara también
...
### Separador transparente (tokens `gap1`, `gap2`…, 2026-08-07; ancho fijo 2026-08-14)
...
- **`DockConfig::isSpringToken()`** es nuevo: `gap` expande igual que `spring`, y varios
  ...
  enciende `fillMain`). En el delegate, `sec.isSpring` es `spring || (gap && !fixedGap)` — o sea
  que todo el layout, el arrastre y la exclusión de etiquetas siguen valiendo sin tocarlos.

#### Ancho fijo, por instancia (2026-08-14)

Cada separador transparente puede **dejar de expandirse y medir lo que le digan**. Como el
ancho es de la instancia y no del dock, los tokens pasaron a estar **numerados**, igual que los
`appsel<n>`: dos `gap` pelados en `widgetOrder` son indistinguibles y no habría por dónde
guardarle el ancho a uno solo.

- **Migración**: `DockConfig::migrateGapTokens()` corre en `load()`, **antes** de
  `reconcileWidgetOrder()`, y convierte cada `"gap"` pelado al `gapN` libre más bajo, de
  izquierda a derecha. Es idempotente (un preset o un backup viejo lo vuelve a traer) y por eso
  `isRepeatableToken()` **sigue aceptando el token pelado**: es la red por si algo lo mete en el
  orden sin pasar por un `load()`.
- **Config**: un grupo por instancia en el `.conf` del dock — `[gap1] fixedWidth=true`,
  `size=60`. Se lee con `gapFixedWidth(token)` / `gapSize(token)` (acotado a
  `kGapMinSize`..`kGapMaxSize`, default `kGapDefaultSize` = 32 px; el spinbox del diálogo lee
  esas constantes en vez de repetir el rango). `insertGap()` devuelve el token nuevo eligiendo
  el número libre más bajo, y `removeSectionAt()` llama a `clearGap()` para que el número vuelva
  a circulación sin arrastrar el ancho del anterior.
- **QML**: los dos getters son `Q_INVOKABLE`, así que ninguna señal los re-evalúa sola. Van
  envueltos en `root.gapFixedOf()` / `root.gapSizeOf()`, que tocan `config.gapRevision` primero
  — el mismo truco que `widgetNameOf()` con `widgetNamesRevision`. `gapComp` es el componente
  nuevo: no dibuja nada, **es** su tamaño (0 mientras expande, `gapSize` px sobre el eje
  principal cuando está fijo, y un ícono de widget en el eje transversal para que la sección
  siga teniendo algo que agarrar).
- **Tres cosas que colgaban de `isSpring` y hubo que separar**, porque un gap fijo ya no lo es:
  la exclusión del nombre (`labelled`, o el nombre se dibuja flotando sobre el agujero), el
  `when` del `Binding` de `hovered` (el componente no declara la property y avisa en cada carga)
  y la visibilidad del ítem *Quitar separador transparente* del menú de sección. La rayita tenue
  de descubrimiento **sigue colgada de `isSpring`**: un gap fijo no la dibuja, a propósito — es
  una línea que quedaría flotando sobre el escritorio.
- **`hasSpring` no cuenta los gaps fijos**: no reparten nada, así que con solo esos la
  alineación vuelve a aplicar (y el combo de la solapa General se re-habilita solo).
- **UI**: solapa *Diseño*, grupo *Separadores transparentes*, una fila por instancia (casilla
  *Ancho fijo* + spinbox en px, gateado por la casilla). Se reconstruye con
  `widgetOrderChanged` —igual que el panel de los `appsel`— porque los separadores se agregan y
  se quitan en la lista de arriba y cambiar de solapa no reconstruye el diálogo. La lista de
  secciones los numera **por su token** (no con un contador posicional, que discreparía con el
  panel apenas se saca uno del medio).
...
- **Solo tiene lugar donde un spring lo tiene**: en modo panel o con `dockLength > 0`. Con el
  largo automático y sin panel, el dock se ajusta a su contenido y el hueco mide 0 px. **Con
  ancho fijo no**: ahí no reparte sobrante, así que anda en cualquier modo.
...
- **En los modos que ocultan, la superficie se pega al borde de la pantalla y el margen lo paga
  el dock** (2026-08-10, rehecho 2026-08-15). La franja que hace reaparecer el dock son 3 px al
  borde de *la superficie* (`applyHiddenMask()`), así que con un margen de pantalla —4 px por
  defecto, 0 solo en modo Compacto— esos píxeles caían **adentro** de la pantalla: apoyar el
  mouse en el borde no hacía nada y el dock aparecía recién al *retirar* el puntero exactamente
  esos píxeles.
  - **El reparto es fijo, no depende de si está escondido**: `DockConfig::edgeInset()` devuelve
    el margen efectivo en los modos 1 y 2 (y 0 en los otros dos, donde nada se mueve nunca), y
    `anchorEdgeMargin()` es lo que queda para el ancla de layer-shell — o sea 0 mientras el dock
    pueda ocultarse. La superficie se ancla pegada al borde y se hace `edgeInset` px **más
    gruesa**; `Dock.qml` dibuja el dock corrido esos píxeles (`root.surfaceThickness`,
    `slider.revealedOffset`), así que el hueco contra el borde se ve igual que siempre, pero es
    una **banda transparente de la propia superficie**. Los márgenes laterales de alineación
    siguen con el margen real, o el dock se correría a lo largo del borde.
  - **Por qué fijo**: la versión anterior era `m_hidden ? 0 : effectiveMargin()`, o sea que la
    superficie se re-anclaba en cada ocultar y en cada mostrar. Mostrar restaura el margen al
    **principio** del deslizamiento (`onRevealedChanged` → `setHidden(false)`), así que el dock
    saltaba `margen` px a mitad de la animación; y peor, con el puntero apoyado en las últimas
    filas de la pantalla —donde está la franja, y donde el puntero queda cuando uno lo tira
    contra el borde— la superficie se corría y lo dejaba **afuera**: se perdía el hover, el dock
    se escondía, el margen volvía a 0, el puntero quedaba otra vez sobre la franja y volvía a
    aparecer. Rebote infinito ("bumping", reportado 2026-08-15). Con la banda, el puntero apoyado
    en el borde sigue **adentro** de la superficie una vez revelado, que es lo que corta el ciclo.
  - **La banda es transparente pero sí recibe input** (la máscara de `applyHiddenMask()` cubre
    toda la superficie cuando el dock está visible): eso es justamente lo que conserva el hover.
    El precio son `margen` px al borde de la pantalla que se come el dock en vez de la ventana de
    abajo, y que `dockRect()` —que ahora pasa `anchorEdgeMargin()`, no `effectiveMargin()`—
    incluye la banda, así que el dodge dispara con una ventana que llega a ella.
  - `setHidden()` **solo** cambia la máscara: llamar a `applyLayerProperties()` desde ahí es
    exactamente lo que no hay que hacer. El grosor de la banda no entra en `dockThickness()`
    (que es la zona exclusiva, y solo el modo 0 reserva, donde el inset es 0).
  - `hideDistance` es `root.surfaceThickness` (contenido + banda): escondido, la superficie
    entera queda fuera de pantalla.
...
- `setHidden(false)` clears the mask, restoring full input. **Nada más**: la superficie no se
  re-ancla en esa transición (ver el bullet del inset en *Modos de ocultamiento*).
- **Dos estados, no uno** (2026-08-15): `root.revealWanted` es lo que el dock *debería* estar
  (hover / menú abierto / arrastre / el modo no pide ocultar) y `root.revealed` lo que está.
  Mostrar es inmediato; ocultar espera `config.hideDelayMs` con `hideDelayTimer`, que al disparar
  **relee** `revealWanted` en vez de asumirlo (el puntero puede volver y volver a irse dentro de
  un mismo intervalo). `revealed` arranca como binding sobre `revealWanted` —así el primer frame
  es correcto en un dock que arranca oculto— y el handler lo rompe en el primer cambio, que es la
  intención.
- **Las dos duraciones son compartidas** (`kdock.conf`, patrón estático de `showTooltips()`):
  `DockConfig::hideAnimationMs()` (default 200, 0 = instantánea) y `hideDelayMs()` (default 0).
  Los dos setters emiten `hideTimingChanged()` en **todas** las instancias vivas, así que el
  spinbox de Configuración → General mueve los docks en vivo. Son compartidas a propósito: es un
  ajuste de tacto, y ponerlo dock por dock en una máquina con doce no es un ajuste.
...
- **Per-panel background image (tiled)**: `config.panelImage` (absolute path, **per-dock**, empty = none) + read-only `panelImageUrl` (`QUrl::fromLocalFile`). `Dock.qml` draws an `Image` (`id: bgImage`) inside the `background` Rectangle, **over** the base color, `fillMode` = `TileHorizontally` (horizontal docks) / `TileVertically` (vertical docks) — scaled to the dock thickness, repeated along its length. Honors `config.opacity`. Rounded corners in non-panel mode via `layer.effect: OpacityMask` (needs `import Qt5Compat.GraphicalEffects`, shared with the neon `Glow`; runtime dep `qml6-module-qt5compat-graphicaleffects`). Chosen via a `QFileDialog` button in Settings → **General → "Panel image"** (+ "Clear"). **Note**: the config stores only the *path*; a preset (and the exported `.zip`) therefore does **not** bundle the image file.
...
- **`refresh()` is asynchronous, and has to stay that way** (2026-08-16). It is five `pactl` queries; they used to run through a `runSync()` with `waitForFinished(800)` each, on the GUI thread — up to four seconds of frozen dock per refresh. It never showed while the audio server answered instantly, and it bit the day an HDMI sink joined the graph: `pactl` went slow *and* `pactl subscribe` flooded events, so the 150 ms debounce bought another four-second freeze as soon as the previous one ended. Now `runAsync()` starts all five in parallel, each with an 800 ms watchdog `QTimer` that kills the child instead of waiting on it, and `finishRefresh()` applies the batch when the last one lands (defaults first — `parseDevices()` compares against them to flag the default device). Overlapping refreshes **coalesce**: one batch in flight, one remembered, so an event storm can never stack processes. Frozen in `tests/unit/tst_audiocontrol.cpp`, which asserts on the event loop still ticking rather than on how long the call takes.
- **Perfiles de tarjeta (2026-08-22)**: la solapa gana un grupo *Audio cards* que conmuta el **perfil** de cada tarjeta. Es lo que hace visible la salida HDMI de un monitor enchufado: en una tarjeta HDA el sink HDMI **no existe** hasta que el perfil activo lo incluye (`output:analog-stereo+input:analog-stereo` solo crea el analógico), y kdock solo lista los sinks del perfil activo — o sea que sin esto el HDMI era invisible bajo una sesión sin plasma-pa (LXQt). `AudioControl::cards()`/`cardList()` y `parseCards()` (una sexta consulta `pactl list cards` en el batch) leen `Card #<n>`, `Name:`, `device.description`, las líneas `Profiles:` —nombre antes del primer espacio, que es el delimitador porque los nombres llevan `:` pero nunca espacio— con `available: yes|no`, y `Active Profile:`. `setCardProfile(index, name)` llama `pactl set-card-profile <índice> <perfil>` y agenda un refresh (el cambio de perfil altera el conjunto de sinks por construcción). La sección `Ports:` del listado resetea el estado del parser, o sus puertos se cuelarían como perfiles. En el diálogo, el combo solo ofrece los perfiles `available` (el activo no disponible se muestra deshabilitado para no mentir); la sección se reconstruye con el resto desde `AudioControl::changed`. Los puertos conectados son exactamente lo que `api.acp.auto-profile=false` deja quieto en PipeWire/WirePlumber 0.5: el autoswitch de perfiles solo existe para Bluetooth. Cubierto por `tst_audiocontrol::cardsAndProfilesAreParsedFromPactl` (el fake de `tests/lib/fakebin.sh` emite una tarjeta de ejemplo para `list cards`).
...
- **Hotplug**: `screenAdded`/`screenRemoved`/`primaryScreenChanged` enter through `scheduleScreenTopologySync()`, a `QTimer::singleShot(0)` coalescer. Así `sync()` ve una topología Qt ya asentada aun cuando una desconexión emita varios eventos; registra los nombres vistos y emite `screenTopologyChanged()` una sola vez por tanda. También se sincroniza al habilitar/deshabilitar. Crea un dock solo si su dockId está habilitado, su pantalla está conectada y `wantedDocks(currentDesktop())` lo selecciona; destruye toda otra instancia viva. Un cambio de **primaria** recrea las instancias afectadas porque solo el dock primario aloja relanzadores. La solapa *Docks* escucha `screenTopologyChanged()` y recarga los estados/nombres aunque el diálogo ya esté abierto. `tst_dockmanager` cubre el agregado y retiro durante ejecución mediante la costura `KDOCK_TEST_SCREENS`, con conectores arbitrarios en vez de una lista física fija.
- **Shared vs per-instance**: volume/brightness/overview/desktop/monitor/wallpaper/power/window-monitor/relanzadores/scriptRunners are shared across all docks (the systray host is no longer among them — it moved to `kdock-systray`; clipboard history moved to the independent `kdock-clipboard`). Per-instance objects (created in `createInstance`): `DockConfig`, `DockModel` and **both clocks** — `ClockWidget`/`ClockWidget2` are per-dock because their 12/24h/date/seconds format is a per-dock config setting. Configs are cached in `m_configs` (keyed by dockId, kept alive even when its live instance is destroyed) so `SettingsDialog` can edit disconnected or inactive docks; `configFor(dockId)` returns the same object the next live dock uses (edits apply live when it exists).
 - **Systray: moved to its own binary** (2026-08-24; before that it was drawn inline by any single dock). The SNI host/watcher + model + menus now live in the resident `kdock-systray` process (see *System tray* below); each dock's `systray` widget is just a launcher button (`SystrayLauncher`) that toggles that window. There is no per-dock exclusivity anymore — several buttons all toggle the same one window — so the old `systrayDockId()`/`systrayDockIdFor()`/`normalizeSystrayOwner()`/`canCoexist()` machinery and the `config.showSystray` flag were all removed. Any dock can carry the button.
...
- **Copiar a Sig. Monitor** (`DockManager::copyDockToNextMonitor`, en el menú *Dock* justo debajo de *Mover Sig. Monitor*, 2026-08-09): lo mismo, pero el dock original **queda donde está y habilitado**; el monitor siguiente recibe una copia. Los dos entran por `cloneToNextMonitor(dockId, keepSource)`, que **es** el cuerpo del move: la única diferencia es qué pasa con el origen una vez que la copia está en su lugar. Tres detalles:
    - **La copia cede la bandeja.** `copySettingsTo()` clona todo, así que la copia nacería con `showSystray=true` y la bandeja se dibujaría dos veces. Cuando origen y copia pueden estar en pantalla a la vez (`canCoexist()`, o sea conjuntos de escritorios que se intersecan) la copia sale con el flag apagado; si están en escritorios disjuntos, las dos lo conservan — la misma regla que `normalizeSystrayOwner()`.
    - **No barre los `.tmp`.** Ese barrido existe para los archivos que deja el *move*; una copia no deja ninguno, así que la rama `keepSource` se lo saltea.
    - **El alias no viaja**, igual que en el move y en *Duplicar para el escritorio*: `copySettingsTo()` saltea la clave `alias` a propósito ("el nombre es del original"), así que la copia nace sin nombre. `DockWindow::copyToNextMonitor()` es el gemelo diferido de `moveToNextMonitor()` y también para el diálogo abierto sobre el dock nuevo (`showMonitorsTab`), que es el que el usuario va a querer editar.

### Ciclo de vida de auxiliares residentes (`src/apprestart.cpp`, 2026-09-01)
- `kdock-previews`, `kdock-tilemenu`, `kdock-controlmanager`, cada `kdock-desktop`, `kdock-clipboard` y `kdock-systray` son binarios separados que pueden quedar residentes mientras kdock está abierto, pero **no** pueden sobrevivir a un *Salir* o *Reiniciar* de toda la familia.
- `kdock::stopAccessories()` es el cierre centralizado: envía `quit()` por D-Bus, espera hasta 2 s a que se liberen los servicios y, solo para los `org.kdock.*` que sigan registrados, obtiene el PID con `servicePid()` y escala a `SIGTERM` (500 ms) y `SIGKILL` (500 ms). No usar `pkill` ni barrer por nombre: el nombre D-Bus identifica exactamente al accesorio de esta sesión.
- `restartAll()` llama ese método antes de relanzar el dock; el hook `aboutToQuit` y las rutas de `SIGTERM`/`SIGHUP`/`SIGINT` también lo llaman. Así el botón *Salir*, logout y una terminación externa no dejan ayudantes viejos. `kdock-calendar` y `kdock-weather` no forman parte de este cierre: no son auxiliares residentes controlados por D-Bus.
- El systray normalmente se oculta, no se termina; aun así debe figurar en esta lista para el cierre total. Si *Precargar* sigue activo, el inicio del nuevo kdock lo vuelve a levantar.
...
- **Diferido y persistente**: un dock que el escritorio actual no quiere **no se destruye su configuración, pero sí su instancia viva**: `DockManager::sync()` conserva el `DockConfig` cacheado y destruye el `DockWindow`, `DockModel`, relojes y `QQmlEngine`. Al entrar en ese escritorio vuelve a construirlo desde el mismo `.conf`. Así una sesión con muchos docks por escritorio virtual no mantiene todos los motores QML y las superficies Wayland vivos a la vez; los docks de monitores desconectados tampoco se instancian.
- **La visibilidad por escritorio ya no usa `DockWindow::setDeskVisible(false)`**. Esa ruta (`hide()` de QtWayland, que destruía la superficie pero conservaba el `QQmlEngine`) quedó fuera del ciclo normal porque dejaba memoria y bindings activos. `setDeskVisible()` sigue disponible para transiciones o consumidores que necesiten mapear una ventana existente, pero `DockManager` solamente mantiene instancias de `wantedDocks(currentDesktop())`.
- **Ciclo de vida y audio (2026-09-01)**: los `.conf` de docks desconectados o ligados a otro escritorio virtual se conservan; solo se liberan los objetos vivos (`DockWindow`, modelo, relojes, superficie Wayland y motor QML). El cambio de escritorio o de monitor no deja docks ocultos reteniendo bindings ni recursos. En `kdock` los backends de audio siguen siendo singleton del proceso —un `AudioControl` y un `VolumeControl`, no uno por dock virtual— y cada `kdock-desktop` activo aporta como máximo su propio `AudioControl`; por eso la cantidad de clientes `pactl subscribe` depende de procesos de monitor reales, no de la cantidad de configuraciones guardadas.
...
### Brillo neón (`config.neonActive`, solapa Neon, 2026-08-25)
- **Un contorno luminoso alrededor de cada panel del dock.** Efecto puramente visual, **global con opt-in por monitor** (como la solapa Desktop, no per-dock). Sin proceso ni backend: se dibuja dentro del propio `Dock.qml`.
- **Dónde vive el estado.** Todo en el `kdock.conf` compartido, con **estáticos** calcados de los globales del modo oscuro: `neonEnabled` (maestro), `neonScreens` (QStringList nativa de conectores encendidos), `neonColor` (hex, default `#00e5ff`), `neonIntensity` (0..1, default 0.8), `neonSize` (px de radio del glow, default 16). Nada se escribe por dock. Los setters terminan en `DockConfig::notifyNeonChanged()`, que emite `neonChanged()` sobre cada `DockConfig` de `s_instances` (repinta todos los docks vivos en caliente, como el modo oscuro).
- **La resolución es `DockConfig::neonActive()`**, con **dos disparadores independientes**: el maestro (`neonEnabled()`) **o** el link con modo oscuro (`neonWithDarkMode() && darkModeActive()`). Si ninguno, apagado. Con alguno prendido, manda la lista por monitor: `neonScreens().contains(screenName())` (un dock sin conector resuelve contra el primario, `QGuiApplication::primaryScreen()`). **Caso especial: lista vacía** → el link de modo oscuro enciende **todos** los monitores (su gracia es "funciona sin configurar"), mientras que el maestro solo, con la lista vacía, no enciende ninguno (como antes de que existiera el link).
- **Link con modo oscuro (`neonWithDarkMode`, casilla en la solapa DarkMode, 2026-08-25).** Un estático más en `kdock.conf`; tildado, el modo oscuro enciende el neón **aunque el maestro esté apagado**. Es un **override de lectura, no una escritura**: nunca toca `neonEnabled`, así que apagar el modo oscuro no pisa el estado que el usuario haya dejado en la solapa Neon. Como `neonActive()` ahora depende de `darkModeActive()`, el constructor de `DockConfig` conecta `darkModeChanged → neonChanged` para que la binding QML de `neonActive` se re-evalúe en cada cambio de modo. La casilla escribe con `setNeonWithDarkMode()`, que termina en `notifyNeonChanged()` (repinta si el modo ya estaba activo).
- **Propiedades de instancia read-only para QML** (todas notifican con `neonChanged`): `neonActive` (bool resuelto — el que QML lee), `neonColor`/`neonIntensity`/`neonSize` (passthrough de los estáticos `neonColorSetting()`/`neonIntensitySetting()`/`neonSizeSetting()` — nombres distintos para no chocar con los getters de instancia, igual que `darkAccent()`→`darkAccentColor()`).
- **Dos estilos: rim (interior) y halo exterior**, elegidos por `neonGlowMode` (0 = rim, default; 1 = halo), estático más del grupo; resuelto por dock con `neonGlowOuter()` (true solo si `neonActive()`, modo 1 **y** el modo no oculta — ver abajo).
- **El rim** (default) se dibuja **por encima** del panel (`z: 3`, dentro del `Rectangle{id:background}`, uno por run): un `Rectangle` de solo borde (fuente `layer.enabled`) blureado con un `Glow` (`layer.effect`, `Qt5Compat.GraphicalEffects`, `spread:0.2`) para el resplandor que se derrama **hacia adentro**, más una línea nítida (`Qt.lighter(neonColor,1.4)`, 2 px) que marca el tubo. Existe porque el panel llenaba **toda** la superficie layer-shell, así que un glow hacia afuera lo recortaba el compositor — que es exactamente lo que el halo exterior resuelve.
- **El resplandor lo dibuja `Glow` de `Qt5Compat.GraphicalEffects`** (`radius:neonSize`, `samples` capados —41 rim / 81 halo—, `spread` 0.2 rim / 0.45 halo, `transparentBorder:true`), aplicado como `layer.effect` sobre el `Rectangle` de solo borde. **Nota histórica**: el 2026-08-26 se migró a `MultiEffect` (`QtQuick.Effects`) por un supuesto ahorro de buffers, pero se percibió más lento en la sesión real y **se revirtió a `Glow` el mismo día**. Si volvés a intentarlo, medí GPU en la sesión Wayland real antes de comitear — el ahorro teórico no se sostuvo. `Qt5Compat.GraphicalEffects` lo comparten el neón y el `OpacityMask` de la imagen de panel.
- **El halo exterior (2026-08-26) le da al glow lugar transparente para derramarse hacia afuera**, reusando la primitiva de los modos de ocultamiento (`edgeInset`/`surfaceThickness`): la **superficie crece más allá del dock** y el panel se **insetea** dentro de ese sobrante transparente, así el `Glow` (dibujado **detrás** del panel, un `Item` por run que crece `glowMargin` por lado con la fuente en la forma del panel) se ve entero. Piezas:
  - **Margen por lado, solo donde el anclaje deja lugar** (`Dock.qml`, props `goInterior`/`goEdge`/`goStart`/`goEnd`): interior siempre; extremos si no están anclados (centrado los libera los dos, `alignment` fija el suyo, panel 100% fija ambos); lado del borde hasta `effectiveMargin()` (un dock **flotante** brilla por los cuatro lados, uno pegado al borde no glow hacia la pared). `neonGlowMargin() = ceil(neonSize)`, `neonEdgeGlow() = min(glowMargin, effectiveMargin())`.
  - **El panel no se mueve.** En el lado del borde eso implica **restar `neonEdgeGlow()` al margen del ancla** (`dockwindow.cpp`, `edgeM`) y crecer la superficie ese tanto hacia el borde; el QML insetea el panel de vuelta.
  - **Zona exclusiva sin tocar** (`thickness()+m`): el halo flota sobre las ventanas vecinas, no las corre.
  - **Región de entrada click-through**: `Dock.qml` empuja los rects de los paneles pintados (en coords de superficie) con `dockWindow.setContentRects()`, y `applyHiddenMask()` enmascara a **su unión** cuando llegan (el margen de glow queda fuera del input). Vacío ⇒ vuelve la máscara por gaps de siempre.
  - **Scope v1: solo en modos que no ocultan.** En auto-hide/dodge `neonGlowOuter()` es false y cae al rim — componer el margen de glow con la banda `edgeInset` del reveal no vale la pena, y un dock que se esconde no necesita un halo permanente.
  - **No crece hacia un monitor vecino** (`DockConfig::screenHasNeighbor(side)`, 2026-08-26): un lado que limita con otra pantalla no glowea, porque la superficie crecida **se derrama sobre el monitor de al lado** — y no solo por el borde: al eje *along*, cuando **cualquiera** de los dos extremos tiene vecino, se le apagan **los dos** (crecer el extremo lejano ensancha la superficie y KWin la centra en el área útil recortada por los struts de los otros docks, empujando el extremo cercano a cruzar el borde — medido: un dock `dockLength=95` en eDP-1 con DP-1 al lado quedaba en `x=42 w=1904 → 1946`, 26 px sobre DP-1). El lado interior/borde, perpendicular a una junta lado-a-lado, no puede cruzarla y sí crece. Un lado sin vecino derrama fuera del escritorio, inofensivo.
- **El `Glow` es un shader de GPU, así que el arnés de Xvfb NO lo muestra** (backend software = sin shaders, el PNG sale con las líneas nítidas del tubo pero sin el resplandor blureado, así que un cambio de render se comprueba con build + qmllint + `qml.smoke` + geometría headless, y el resplandor en sí se mira en la sesión real). Headless se verifica la **geometría** (el tamaño de la superficie crece exacto: un dock centrado flotante 768×68 pasa a 804×104 con `neonSize=18`; `alignment`/panel/auto-hide dan los tamaños de la matriz), la lógica (`neonGlowOuter`/`neonGlowMargin`/`neonEdgeGlow` con una sonda que linkea `kdock_core`) y la persistencia; el resplandor en sí solo se ve en la sesión Wayland real con GPU (verificado 2026-08-26: el halo se derrama por los cuatro lados de un dock flotante, el rim no).
- **UI (solapa)**: solapa **Neon** (`SettingsDialog::createNeonTab()`, junto a DarkMode) con el checkbox maestro *Activar Neon*, el selector de color (`makeColorButton` con `neonColorSetting`/`setNeonColor`), dos `QSlider` (intensidad 0-100→0..1, tamaño 0-40 px), un combo **Estilo** (*Rim (contorno interior)* / *Halo exterior*, ↔ `neonGlowMode`) y el `QListWidget` de monitores calcado del de Desktop (filas checkables sembradas de `neonScreenEnabled()`, greyed cuando el maestro está apagado). Sin refresco de ksycoca ni checklist de siete archivos: es config-only + render.
- **UI (clic derecho)**: submenú **`qml/NeonMenu.qml`** (`title "Neon"`, ícono `draw-highlight`), instanciado tras `ModeMenu` en los **dos** menús contextuales de `Dock.qml` (el del ícono y el de la sección de widgets), como `BackgroundColorMenu`/`ModeMenu`. Tres cosas, **a propósito solo acá y no en el diálogo**: (1) **cinco colores rápidos** (`config.neonPresetColors` = `DockConfig::neonQuickColors()`, hexes fijos) → `config.setNeonColorGlobal()`, que es el estático global, así que cambiar el color cambia **todos** los docks; (2) **maestro global** (`config.neonEnabledGlobal` ↔ `setNeonEnabledGlobal`); (3) **opt-out por dock** (`config.neonDockDisabled`, WRITE), la feature nueva: una clave **per-dock** (`neonDisabled` en el `.conf` del dock, en `load()`) que en `neonActive()` **gana sobre todo** (maestro, link de modo oscuro). Su setter emite `neonChanged()` **solo en esa instancia** (es per-dock, no global). Los tres exponen a QML vía `Q_INVOKABLE`/Q_PROPERTY de instancia que delegan en los estáticos, para que el QML escriba natural (`config.setNeonColorGlobal(...)`, `config.neonDockDisabled = ...`).
...
### ColorAuto — esquema de color desde el wallpaper (`src/wallpapercolors.{h,cpp}` + `src/autocolorscheme.{h,cpp}`, 2026-08-12)

- **Qué hace**: saca el color predominante del fondo de pantalla, arma con él un esquema de color de KDE 6 válido y lo aplica — al escritorio entero y/o a los docks, cada uno con el fondo de **su** monitor. Se configura en la solapa **ColorAuto**, que es **app-wide**: no hay nada por dock.
- **Está partido en dos a propósito.** `WallpaperColors` es **puro** (imagen → colores → archivo; ni D-Bus, ni config, ni UI) y por eso es lo único que se puede probar en CI: `tests/unit/tst_wallpapercolors.cpp`, 21 casos sin sesión, sin KWin y sin Plasma. `AutoColorScheme` es todo lo demás: ajustes, triggers, la lectura por D-Bus y el enclavamiento con el modo oscuro (`tests/unit/tst_autocolorscheme.cpp`, 16 casos).
- **El muestreo es el mismo algoritmo del color dominante de un ícono** (`IconColorProvider::compute`): cubetas de 3 bits por canal con un portón de saturación/valor que descarta gris, negro y blanco, gana la más poblada, y un acumulador de respaldo con el promedio de todos los píxeles para un fondo monocromo. Copiado a propósito: ya se sabe que se ve bien acá.
  - **La velocidad no está en el bucle de píxeles, está en `QImageReader::setScaledSize()`.** Para un JPEG eso pasa por el `scale_denom` de libjpeg, así que un fondo 4K nunca se materializa entero. Con `QImage::load()` el bucle sería igual de rápido y la función diez veces más lenta.
  - Caché por `path|mtime|size`: un pase de diapositivas que reescribe la misma ruta **sí** se vuelve a muestrear (si no, el esquema se congelaría en el primer fondo del día), y un fondo que no cambió no se decodifica dos veces.
  - La luminancia media sale de la **misma pasada**, gratis, y es lo que decide claro u oscuro. El combo *Automático / Siempre claro / Siempre oscuro* existe porque una foto justo en el umbral hace titilar el esquema en cada paso del pase.
- **La derivación es primero contraste, después color.** Los fondos son el **tono** de la semilla a la saturación y el valor de la banda del esquema (el fondo *es* el wallpaper sin ser estridente); todo lo demás se empuja hasta cumplir una razón WCAG contra lo que tiene debajo: 7.0 para el texto (AAA de cuerpo), 1.2 para que el botón se lea como superficie aparte, 3.0 para que la selección no se funda con la ventana, 4.5 para los semánticos. `ensureContrast()` mueve valor primero y saturación después, y si el objetivo es inalcanzable devuelve **lo mejor que alcanzó** en vez de fingir. Eso es lo que hace que salga legible con cualquier foto, y es lo que afirma el test sobre las cuatro imágenes × tres claridades.
  - Los semánticos (`ForegroundNegative/Neutral/Positive/Link/Visited`) son los de Breeze y **no siguen al wallpaper**: un "negativo" que no es rojo deja de significar negativo. Solo se los empuja por contraste.
- **El color de selección**, por omisión, es lo pedido: esquema claro → gris oscuro `#3A3A3A`; esquema oscuro → gris claro `#D6D6D6`. La **letra no se configura**: sale sola de la regla de contraste (blanca sobre el gris oscuro, negra sobre el claro), así que no es un caso especial. Los otros dos modos son *Color propio* (dos swatches, uno por caso) y *Del fondo* (tono complementario, saturado a la fuerza porque la semilla puede ser casi gris).
- **`plasma-apply-colorscheme` NO hace nada si el nombre pedido ya es el puesto**, y esto manda todo el diseño del apply. Verificado en el fuente (`kcms/colors/plasma-apply-colorscheme.cpp`: imprime "ya está aplicado" y vuelve). O sea que reescribir un `.colors` en su lugar y re-aplicarlo es un **no-op silencioso**: la feature se congelaría en el primer fondo sin un solo error. Por eso el esquema **alterna entre dos nombres** (`KdockColorAuto1` / `KdockColorAuto2`), que es lo mismo que hace kde-material-you-colors.
  - Su costo conocido son dos esquemas de sobra en la lista de Preferencias del Sistema; acá el que dejó de ser el actual se **borra 3 s después**, así que en reposo queda uno solo. El borrado va diferido porque la herramienta es `startDetached` y sigue enumerando el directorio un rato después de que volvimos.
- **Los docks se pintan con un override de lectura, igual que el modo oscuro** — y por la misma razón: el `panelColor` del usuario queda intacto en su `.conf` y apagar la feature restaura por construcción, sin snapshot que se pueda desincronizar.
  - **Ese override le gana al `panelColor`, así que limpiarlo mal deja el dock inservible** (2026-08-12, reportado). Los colores se ponían recorriendo `enabledDocks()` y se sacaban recorriendo `knownDocks()`: dos listas que en una config real diferían en **cuatro docks**, y esos cuatro quedaban con un color generado que después **no había forma de cambiar** — ni el menú contextual ni el diálogo, porque los dos escriben `panelColor` y la rama de ColorAuto va antes. Ahora se limpia por `DockManager::liveConfigs()`, o sea los `DockConfig` que el manager repartió: los colores solo llegan por `configFor()`, que cachea, así que ese conjunto es exactamente el que puede tener uno y no se puede desincronizar. Corolario general: **cuando un override de lectura tapa una opción del usuario, el conjunto que se limpia tiene que derivarse del que se pinta, no de otra lista que "debería" coincidir.**
  - Y cada refresco **limpia los docks cuyo monitor no pudo resolver**, en vez de dejarles el color anterior: un monitor desenchufado, o cuyo fondo dejó de leerse, se quedaba pegado a su último color por lo que durara el proceso. `DockConfig` suma `autoColorActive` / `autoBackground` / `autoAccent`, **no persistidas** (`setAutoColors()` no toca `QSettings`), y `Dock.qml` las lee en `dockBaseColor`, `dockTextColor` y `widgetIconTheme`. El modo oscuro gana la precedencia, que es gratis porque ColorAuto se apaga solo mientras aquel está puesto.
  - `widgetIconTheme` con ColorAuto sigue a `dockBaseIsLight` como el modo 1: una elección fija de iconset se vuelve ilegible en cuanto un fondo claro sigue a uno oscuro.
- **El trigger que importa es el watcher del `appletsrc`** (`QFileSystemWatcher` sobre `plasma-org.kde.plasma.desktop-appletsrc`), agregado en la v2 después de que la feature llegara a producción sin él. Los cuatro triggers en proceso —el widget/atajo de *siguiente fondo* (`WallpaperControl::wallpaperAdvanced`), los wallpapers por escritorio virtual (`DesktopWallpapers::wallpapersApplied`), el cambio de escritorio y *Aplicar ahora*— solo ven los cambios que **hizo kdock**, y esa resultó ser la minoría de los casos: un script del Script Runner hablando `qdbus6` derecho a plasmashell, el pase de diapositivas de KDE avanzando solo o Preferencias del Sistema dejan a kdock enteramente afuera. El síntoma reportado fue "generó una vez y nunca más". Todos desembocan en `refresh()`, con un `QTimer` de 1200 ms que coalesce.
  - **Y `kdock --next-wallpaper` tampoco alcanzaba**: ese CLI corre en un **proceso kdock aparte** (`runNextWallpaperCli`, antes de construir ningún dock) que emite la señal dentro de sí mismo y sale. El dock que está corriendo no se entera. O sea que ni el camino "oficial" cerraba el círculo.
  - **Hay que re-agregar el path en cada señal.** KConfig guarda escribiendo un temporal y renombrándolo encima, así que el inodo vigilado desaparece y `QFileSystemWatcher` **suelta el watch en silencio**: sin re-armarlo funciona exactamente una vez, que es el mismo síntoma que el watcher viene a arreglar. Se vigila también el **directorio**, porque entre el `unlink` y el `rename` el archivo no existe.
  - **El watcher no se apaga con `enabled()`.** El gate vive en `refresh()`; ponerlo en el watcher haría que prender la casilla no reaccione hasta el *siguiente* cambio de fondo.
  - **Pero el evento del directorio NO puede ser el disparador, y por eso hay `appletsrcChanged()`** (2026-08-14, bug reportado como "flickeo en el diálogo + el sistema pesado"). Ese directorio es `~/.config`, y `applySystem()` lanza `plasma-apply-colorscheme`, que **escribe `kdeglobals` ahí adentro**: el `directoryChanged` volvía como un cambio de fondo y cerraba un bucle sin salida — aplicar → kdeglobals → refresh → aplicar, **un cambio de esquema de color completo cada ~2 s, para siempre**. El ping-pong `KdockColorAuto1/2` es justamente lo que hace que cada vuelta sea un apply de verdad. Los tres síntomas salían de ahí: plasmashell repintando el escritorio en bucle (`serializeImage`, con un `evaluateScript` sobre 75 containments cada 2 s), el `Theme` de kdock releyendo `kdeglobals` → `QApplication::setPalette()` → **el diálogo de Configuración repintándose entero cada 2 s** (que es el "flickeo"), y `++Theme::m_revision` → todos los íconos de los 12 docks re-resueltos. Ahora los dos handlers re-arman el watch y solo llaman a `refresh()` si la marca `(mtime, size)` del `appletsrc` **propio** cambió.
  - **Y una segunda reja, que es la que cubre la clase entera: `applyPalettes()` no re-aplica lo mismo.** Recuerda las imágenes por conector de la última aplicación automática más una `optionsKey()` con todo lo que cambia el resultado (variante, claridad, modo y colores de selección, iconsets, qué monitor manda, si pinta docks, el iconset default): si una corrida automática resuelve el mismo par, vuelve sin escribir un `.colors` ni lanzar una herramienta. Que la clave incluya las opciones es lo que hace que mover cualquier control de la solapa la invalide sola, sin tener que acordarse de limpiar el memo en cada setter; `restoreDefaults()` sí lo limpia, porque ahí el escritorio deja de tener nuestro esquema. Un `generateNow()` está exento (`m_manualRun`), que es lo que deja pedir la variante siguiente del mismo fondo. Vale como regla general: **cualquier cosa que reaccione a un archivo del sistema tiene que ser idempotente, porque el disparo espurio es el caso normal**. Medido en esta sesión el 2026-08-14: en **7 minutos** Plasma reescribió el `appletsrc` **dos veces** dejando las rutas de los fondos **idénticas**, o sea dos regeneraciones y dos cambios de esquema completos que con la guarda cuestan una lectura. De paso, la anatomía del archivo: **6 containments** (4 escritorios `org.kde.plasma.folder` —uno por pantalla, dos de ellas desconectadas y con `d.screen == -1`, que el script saltea— y 2 paneles) en 76 grupos de INI; contar los grupos y llamarlos containments es un error fácil y lleva a diagnosticar de más.
  - **Los 1200 ms no son cosméticos.** Un avance de pase de diapositivas lo resuelve KDE de forma asíncrona —kdock solo cicla el plugin—, así que leer enseguida devuelve la ruta **vieja**. Mismo orden de magnitud que los ~900 ms de `startDetached` de las herramientas de Plasma.
  - **La lectura es asíncrona, así que puede llegar tarde a una fiesta que terminó**: si el usuario apaga la casilla (o prende el modo oscuro) mientras el viaje de ida y vuelta está en el aire, `applyPalettes()` se planta. Sin esa guarda, la respuesta aplicaba un esquema **después** de que `restoreDefaults()` ya había devuelto el del usuario.

- **Generar a mano: `generateNow()`**, el único punto de entrada de los tres botones (la solapa, el widget `colorauto` y la tarjeta del panel). **No está gateado por `enabled()`** a propósito: la casilla es el modo *automático*, esto es el manual, y lo que deja puesto es del usuario para que lo gestione. Captura los defaults si no hay (si no, se queda sin forma de volver) y marca `ColorAuto/manual`, que es lo que impide que el rescate de arranque —`applied() && !enabled()`— deshaga una elección explícita del usuario **en cada reinicio del dock**. El modo oscuro sí lo frena: mientras está puesto es dueño de la apariencia.
- **Repetirlo cicla colores alternativos.** `WallpaperPalette::candidates` trae las cubetas más votadas y `Options::variant` elige cuál semilla. El contador (`ColorAuto/variant`) sube en cada generación manual y **se resetea cuando cambia la ruta del fondo** del monitor que manda (`ColorAuto/lastImage`) — si no, después de cambiar de fondo seguirías en la variante 7 del anterior.
  - **Un fondo monocromático igual tiene que dar variantes.** Medido sobre la captura de pantalla que el usuario tenía puesta: **56 cubetas vívidas, todas en el tono 202°**. Con la separación por tono eso da **una sola** candidata, y ocho clics daban ocho veces el mismo esquema — que es exactamente lo que se reportó como "el botón deja de funcionar". Dos correcciones: el tope de `kMaxCandidates` se aplica a lo que **sale**, no a cuántas cubetas se miran (antes se miraban solo las 8 más votadas, todas del mismo tono), y la lista se completa con **armonías** del dominante (complementario, tríadas, etc.: su propio tono rotado por los intervalos clásicos). Las del fondo van primero; las armonías siguen siendo "el color de esa foto, en otra nota".
  - **Las candidatas se separan por tono** (`kMinHueDistance`, 25°), y sin eso la feature no se ve. Las cubetas más votadas de una foto suelen ser tres matices del mismo color —medido sobre un fondo real: (25,39,47), (71,116,143) y (51,87,109), todos el mismo azul— y como el fondo se arma con el **tono** a saturación y valor fijos, ciclar entre ellas daba fondos separados por un punto: `33,37,40` contra `33,38,40`. Con la separación, el mismo fondo da cuatro tonos distintos de verdad.
  - Corolario para diagnosticar: **la dominante es la más votada entre los píxeles *vívidos*, no el promedio de la imagen.** Una foto cuya media es cálida (146,93,63) puede tener como dominante un azul oscuro (1,22,48) con seis veces más votos, porque las zonas cálidas son poco saturadas y el portón las descarta. Eso es correcto y es el mismo criterio que `IconColorProvider`.
...
- **`applied` va persistido**, con el mismo razonamiento que `darkAppearanceApplied`: describe al *sistema*, que sobrevive al proceso. Una corrida que murió con un esquema generado puesto queda deshecha en el próximo arranque; sin eso el usuario se queda clavado en un esquema temporal para siempre.
- **Los defaults se capturan la primera vez que se activa** y son lo que se restaura al desactivar: el esquema de color de KDE y el override de iconset del dock. Ese override se guarda **verbatim, vacío incluido** — vacío significa "seguir el de KDE", y guardar el id resuelto convertiría un dock que seguía al sistema en uno clavado. `captureDefaults()` nunca guarda uno de los nuestros. Y sobre *"el iconset del primer dock del monitor principal (limitación aceptada)"*: **esa limitación no existe**, el override de iconset de kdock es de proceso (clave `iconTheme` del `kdock.conf` compartido), o sea que hay uno solo y es el de todos los docks.
- **Tres puntos de entrada para generar a mano**, todos contra el mismo `generateNow()`:
  - **La solapa**: los botones *Generar Color* y *Guardar Color Scheme*, arriba del interruptor maestro porque tienen que andar con ColorAuto apagado. *Aplicar ahora* queda con el resto: es el "releer el fondo" del modo automático, otra cosa.
...
### Modo QT — la apariencia de KDE aplicada a la sesión LXQt (`src/qtcompat.{h,cpp}`, 2026-08-19; iconset y fuentes 2026-08-20)

- **Qué problema resuelve.** Todos los selectores de esquema de color y de iconset de kdock terminan escribiendo `kdeglobals` (`plasma-apply-colorscheme` / `plasma-changeicons`), y kdock lo lee a mano (`Theme`). Bajo Plasma eso alcanzaba: `plasma-integration` le arma la `QPalette` y el tema de íconos a cada app desde ese archivo. Bajo **LXQt** el tema de plataforma es `lxqt`, que **nunca mira `kdeglobals`**, así que el resto de las apps Qt del escritorio se quedaban con lo que hubiera dejado `lxqt-config-appearance`.
- **El puente son los ajustes del propio tema de plataforma de LXQt.** Leyendo su fuente (`lxqt-qtplugin/src/lxqtplatformtheme.cpp`, en esta máquina el `.so` **no viene de dpkg**: es el build propio del usuario, fuente en `~/IA/apps-nuevas/lxqt-wayland/`):
  - `loadSettings()` lee **diez** claves del grupo `[Palette]` de `~/.config/lxqt/lxqt.conf` —arma `QPalette(window_color)` y pisa los roles que sean válidos (deriva `AlternateBase` del base y los `Disabled` con alpha 130)—, más `icon_theme` al nivel raíz y `font` / `fixedFont` en `[Qt]`;
  - tiene un `QFileSystemWatcher` sobre el archivo y en `onSettingsChanged()` hace `QApplication::setPalette()` + `style()->polish()`, `XdgIconLoader::updateSystemTheme()` y `QApplication::setFont()`, cada uno **solo si esa parte se movió**. O sea que **escribir esas claves re-tematiza en vivo todas las apps Qt ya abiertas**: sin variables de entorno, sin relanzar nada, sin qt6ct.
...
- **Las otras dos partes** (2026-08-20), cada una con su casilla, todas bajo el interruptor maestro:
  - **Iconset**: mismo espejo, `kdeglobals Icons/Theme` → `icon_theme`. Esa clave va **al nivel raíz** de `QSettings`, que es la sección `[General]` del archivo —de ahí la lee el plugin—, y no lleva el prefijo de ningún grupo. Con `kdeglobals` sin iconset **no se escribe nada**: la clave vacía llevaría el escritorio al `oxygen` por omisión del plugin, que no es lo que "no se pudo leer" tiene que hacer. Ojo con el override de iconset propio de kdock (proceso entero, clave `iconTheme` del `kdock.conf` compartido): le gana a esto **para los íconos del dock**, y no viaja a LXQt.
  - **Fuentes** (`[Qt] font` y `fixedFont`): **la única parte con valor propio**, y tiene que serlo — kdock no tiene ninguna fuente de aplicación que copiar, así que no hay espejo posible. Se guardan en `[QtCompat]` del `kdock.conf` compartido, **verbatim en formato `QFont::toString()`**, que es la misma codificación de `lxqt.conf`: nada se parsea ni se re-serializa por el camino, así que un peso o un nombre de estilo que kdock no entienda sobrevive el viaje. Vacío = "dejale la suya a LXQt", **no** "borrala", y por eso arrancan vacías y la casilla arranca apagada. El selector abre en `fontFor()`, que cae a lo que LXQt ya tiene: abriéndolo en la fuente por omisión de Qt, el primer OK le cambiaría la fuente al escritorio a una que el usuario nunca eligió.
  - **kdock toma la fuente como cualquier otra app Qt**: su texto QML usa la fuente de la aplicación, y el tema de plataforma **de su propio proceso** reacciona al mismo archivo. Eso es lo que cubre el "además de kdock" sin una línea de código extra.
- **La mitad que le habla a las apps de KDE: `[UiSettings] ColorScheme`** (2026-08-20, bug reportado por el usuario con Dolphin). Esto **no es sobre LXQt** y es lo que hacía que la feature pareciera no servir para nada: `plasma-apply-colorscheme` escribe `[General] ColorScheme` y los grupos `Colors:*`, pero **nunca** `[UiSettings] ColorScheme` — y `KColorSchemeManager`, que construye toda app KXmlGui, lee **solo esa última**. Sin ella decide que no hay nada configurado y **pisa la paleta que el tema de plataforma acababa de dar** con su Breeze Light compilado adentro (`#eff0f1` / `#ffffff`). Bajo Plasma nunca se vio porque el tema de plataforma es `plasma-integration`; bajo LXQt es el bug entero. `QtCompat::syncKdeUiSettings()` copia la primera sobre la segunda.
  - **Las dos claves llevan el nombre de ARCHIVO del esquema** (`BreezeDark`, no `Breeze Dark`) — medido corriendo la herramienta contra un `XDG_CONFIG_HOME` descartable —, así que la copia es verbatim. Con un nombre que no matchea ningún `.colors`, `KColorSchemeManager` no pisa nada (inofensivo).
   - Se escribe con **`kwriteconfig6`**, no con `QSettings`: `kdeglobals` es un archivo de KConfig, con sintaxis que `QSettings` no conoce (`[$Version]`, los marcadores `[$e]`/`[$i]`, claves localizadas `Name[es]=`). Es la misma razón por la que `AppearanceControl` usa esa herramienta.
...
- **Propagación del entorno de activación (2026-08-21):** `QtCompat::updateActivationEnvironment()` hace `dbus-update-activation-environment --systemd <4 claves>` (fallback `systemctl --user import-environment <4 claves>`) en cada arranque bajo LXQt (`main.cpp` después del `setEnabled`). Sin esto, los lanzamientos vía GIO/systemd (handlers `magnet:`, autostart) heredan el entorno mínimo de `systemd --user` (28 vars, sin `QT_QPA_PLATFORMTHEME`) aunque `systemctl --user show-environment` ya tenga las 40 vars; `ktorrent` arrancó así y ni siquiera cargó `liblxqt.so`, dependiendo solo de `KColorSchemeManager` — que antes del fix sincrónico tampoco tenía `UiSettings` a tiempo. `portal-kde` (pid 5266) sí tenía el entorno completo, de ahí que `dolphin` sí se viera bien.
  - **La lista es explícita y `--all` está proscrito** (fix 2026-08-21, ver el bug de spectacle abajo). Ese flag sube el entorno **entero** del proceso al almacén de activación de la sesión, que **le sobrevive**: un kdock arrancado desde uno de los arneses de Xvfb publicó `DISPLAY=:99`, `QT_QPA_PLATFORM=xcb`, el `XAUTHORITY` de un `xvfb-run` muerto, el `PATH` de los fakes y `XDG_{CONFIG,DATA,CACHE}_HOME` de un sandbox descartable a **todas** las apps activadas por D-Bus. Spectacle se activa así, y por eso abortaba con "could not connect to display :99" en cada captura hasta que el entorno se reparó a mano. Las claves que se publican son las que el tema de plataforma necesita y son **idénticas para toda la sesión**: `QT_QPA_PLATFORMTHEME`, `QT_PLATFORM_PLUGIN`, `QT_QUICK_CONTROLS_STYLE`, `XDG_CURRENT_DESKTOP`. Las de sandbox (`XDG_DATA_HOME`/`XDG_CONFIG_HOME`/…) quedan afuera a propósito: cada arnés de CLAUDE.md las overridea, y publicar un directorio descartable sesión-por-medio es el mismo bug con otra variable.
  - **Y una reja más temprana: solo el dock de la sesión real puede hablar por la sesión.** La función corta salvo que `QGuiApplication::platformName()` sea `wayland`. Bajo Xvfb la plataforma es `xcb` (`tests/lib/xvfb-app.sh` la fija), así que una corrida de test que aisló su config no tiene nada que publicar. Costo conocido: un LXQt **X11** real (plataforma `xcb`) también se saltea la propagación — aceptado, porque distinguir "Xvfb del arnés" de "X11 real" por la plataforma es imposible, y el bug de spectacle vale más que la cosmética de ese caso.
  - **El bug que motivó todo (2026-08-21)**: kdock + el arnés contaminaron `systemd --user` y el almacén de D-Bus con `DISPLAY=:99`/`XDG_*` del sandbox `KDOCK_SANDBOX=/tmp/kdock-smoke-ventanas-abajo-es.*` (que ya no existía). La reparación fue a mano (`systemctl --user set-environment/unset-environment` + `dbus-update-activation-environment` con los valores reales) y luego el fix de arriba. **Si vuelve a pasar, el diagnóstico es el mismo**: `systemctl --user show-environment` comparado contra `/proc/<pid lxqt-session>/environ`, y `pgrep -ax kdock` para saber si el binario que corre todavía es el viejo con `--all`.
...
- **El flujo de refresco es en un solo sentido, y no es un detalle de estilo** (arreglado 2026-08-21, el primer clic en *Activar* mataba el dock con SIGSEGV — ver `CLAUDE-TRAMPS.md` → *Un repintado que se cuelga de `changed()`…*). `refresh()` es lo único que hace `scan()`, guarda el caché y **emite `changed()` sólo si el resultado difiere**; `rebuildKWinScriptsList()` es una vista pura que lee `scripts()` (el caché) y nunca refresca; todos los botones y los `singleShot` post-acción llaman a `refresh()`, no al repintado. La bandera `m_kwinScriptsRebuilding` es la segunda cerradura sobre la misma puerta. El watcher del directorio (`~/.config`, compartido con toda la sesión) dispara sólo si la marca `(mtime, size)` de `kwinrc` cambió: sin eso, cualquier app escribiendo cualquier config costaba un `scan()` completo con sus llamadas D-Bus bloqueantes en el hilo de la GUI.
- **Tests**: `tests/unit/tst_kwinscripts.cpp` (tier `unit`) siembra un `metadata.json` en el sandbox y congela las dos direcciones — un oyente que refresca desde `changed()` converge, y un scan distinto sí emite. No cubre el repintado del diálogo: eso corta en `!available()` sin `org.kde.KWin` en el bus, o sea siempre en CI, y se prueba con la sonda de los `.o` en la sesión real.
...
- **Per-monitor mechanism** (`src/wallpapercontrol.cpp`): drive Plasma's scripting API `org.kde.PlasmaShell.evaluateScript`. Map `screenName`→Plasma screen index by matching `QScreen::geometry()` top-left against `screenGeometry(i)` (scanning only `0..screenCount-1`, so a disconnected containment with `screen == -1` never matches). Then, on the containment with that `.screen`, **cycle its wallpaper plugin**: set `wallpaperPlugin='org.kde.image'` (deliberately *without* `reloadConfig()`, which would repaint to the stale image of that group — a visible extra change), and in a **separate** call set it back to `'org.kde.slideshow'` + `reloadConfig()`. KDE advances the slideshow itself, exactly like the desktop's own *Next Wallpaper*. Two chained async `evaluateScript` calls (the reply of the first is what serializes them).
  - **It used to compute the next image itself and write `Image` + `reloadConfig()`, and that is the bug the user reported on 2026-08-10** ("cambia una vez y después ya no cambia nada"). Measured live on this Plasma 6: three advances in a row moved the stored `Image` key every time (526869 → 526870 → 527496 → 528006) and left **the same wallpaper on screen** — 31 k of 2.07 M pixels differing on that monitor, i.e. the dock's own clock. The plugin cycle repaints on every click (2.03 M pixels, four different wallpapers in four clicks). So: writing a concrete `Image` **does not repaint**, and neither does flipping the plugin twice inside one script; only the two-call cycle does. This is the same finding `next-wall.sh` has been built on since 2026-07-16 — the widget was the last caller still using the old path. `evaluateScript` returns whatever the script `print()`s (not the last expression).
...
- **Snapshot genérico y persistido**: la captura sale de un `evaluateScript` que recorre los
  ...
  rutas y nombres de clave arbitrarios, y de paso entra sola en los presets. **Se fusiona, nunca
...
### Clipboard history — binario accesorio `kdock-clipboard` (`clipboard/`)
- The dock's `clipboard` token is now only a launcher icon (`ClipboardLauncher`). kdock no longer constructs `ClipboardHistory`, `WaylandClipboard` or any clipboard QML, so a clipboard transfer, image decoder or UI failure cannot tear down the dock.
- `kdock-clipboard` owns the session-bus name `org.kdock.Clipboard`, is started/toggled asynchronously by the launcher, and keeps its own `ClipboardHistory` instance. It captures through `ext-data-control-v1` (with QClipboard fallback) and persists the existing JSON/PNG/text history. Corre aislado del proceso principal para su trabajo normal, pero `apprestart` lo cierra junto con el resto de auxiliares ante *Salir* o *Reiniciar* de kdock.
- The window is a normal resizable `QQuickView`, not layer-shell. The launcher passes the monitor, dock edge and the clicked widget's screen point; the accessory clamps an initial placement next to that point instead of centering it. Wayland may still adjust a normal toplevel's final position.
- `Clipboard.qml` provides search, text/image rows, a **Fija** checkbox (`Qt::WindowStaysOnTopHint`), a **Cerrar después de copiar** checkbox (off by default means hide after copying), and an explicit **Cerrar** button. Hiding keeps the accessory resident and capturing; closing quits only `kdock-clipboard`.
- The window size, fixed state and close-after-copy choice live in `~/.local/share/kdock/clipboard.conf`; unlike the old popup there are no clipboard width/height keys in `DockConfig`.
...
- **Búsqueda (listado, orden, historial)** (2026-08-24): al escribir en el buscador los
  ...
  (`searchResults()`), así las dos vistas coinciden. **Está memoizado por `(query, searchSort)`**
  (2026-08-26): sus tres consumidores —`reloadApps()` + `searchPlacement()` + el `searchResults()`
  que ata la lista en QML— corrían la búsqueda+orden entera dos o tres veces por tecla. Ahora el
  cómputo real vive en `computeSortedSearchApps()` y `sortedSearchApps()` devuelve el caché;
  `refresh()` lo invalida al tope, que es por donde pasa todo cambio de estado (query, config,
  menú/favoritos), así que el `searchResults()` que sirve después es fresco.
...
- **UI**: Configuración → **Menu** → grupo *"Menú de mosaicos (pantalla completa)"* (casillero
  ...
  **no** es una solapa nueva: con once títulos la barra ya pide 1086 px y la duodécima la manda
  ...
  Disposición).
- **Presets**: `ConfigArchive` centraliza ocho familias en `configPrefixes()` (`kdock`,
  `previews`, `tilemenu`, `controlmanager`, `weather`, `desktop`, `systray`, `clipboard`) y
  archiva sus `*.conf`, más las traducciones directas `translations/*.md`. Al importar solo se
  borran las familias presentes y las traducciones solo se reemplazan si el `.zip` trae su
  marcador, para que un archivo viejo no borre configuración nueva ni traducciones locales.
...
- **La tarjeta del reloj lleva al calendario con un clic** (`ClockCard.qml`, 2026-08-10):
  ...
  "calendar"`. La `MouseArea` va **dentro** de la tarjeta, no en la cabecera: el `clock` es la
  única sección sin solapa propia (`tab=false`), así que su cabecera no tiene a dónde ir con
  el doble clic, y el cuerpo del reloj no tiene ningún control que se le pise. Se apaga sola
  (`enabled`) cuando el calendario no está entre las solapas visibles, porque poner un
  `currentTab` que la barra no dibuja deja el panel en blanco.
...
- **Celdas rectangulares en Principal** (`cmConfig.cellHeight`, 2026-08-08): las tarjetas de
  la grilla se dimensionan en celdas (span × vspan), y la celda ahora puede ser más alta que
  ancha — "Ancho de celda:" (`cellSize`, con el estirar de siempre) + "Alto de celda:"
  (`cellHeight`, fijo) en **Grilla**. En el QML todo lo que era `pitch` se partió en
...
- **Un binario por monitor conectado** (2026-08-24): en vez de una instancia única, corre un
  ...
  - **Arranque lazy y candado por monitor** (2026-09-01): `applyState()` solo intenta arrancar
    un canvas si el monitor aparece actualmente en la lista de conectados y está en
    `enabledScreens`; una entrada persistida para un monitor desenchufado no crea ningún proceso.
    `launchOn()` mantiene una reserva local por conector durante la ventana entre
    `startDetached()` y `CmService::registerOnBus()` (sondeos de 100 ms durante hasta 5 s), y
    `quitOn()` deja una parada pendiente si el monitor desaparece mientras el hijo arranca. Así
    callbacks simultáneos de hotplug/configuración no pueden producir dos `kdock-desktop` para
    el mismo monitor, y un hijo que llegue tarde se cierra en cuanto reclama su servicio.
  - **Audio acotado al monitor activo**: cada `kdock-desktop` crea un solo `AudioControl`, que
    abre un único `pactl subscribe`; no se crea una instancia por escritorio virtual ni por
    entrada persistida. El `kdock` principal conserva sus dos suscriptores compartidos
    (`AudioControl` + `VolumeControl`) independientemente de cuántos `DockWindow` estén
    configurados. `kdock` no inicia WirePlumber: usa `pactl`/`wpctl` como clientes del servicio
    global del sistema.
...
### System tray — binario accesorio `kdock-systray` (`systray/`, 2026-08-24)

Octavo binario. Antes la bandeja se dibujaba **inline dentro del dock** y kdock era el host (y,
bajo LXQt/wlroots, el watcher) de SNI. Dos problemas la movieron afuera: (1) los menús de los
ítems son `xdg_popup` y dentro del dock no tenían una superficie propia a la que colgarse; (2)
en la sesión mixta el widget aparecía **vacío y oculto** por un split-brain del watcher (ver
punto 12 — kded6 acapara `org.kde.StatusNotifierWatcher` sin servir el objeto). La decisión del
usuario: sacar **todo** el código de bandeja de kdock y ponerlo en un binario propio con ventana
propia.

Reparto:
- **En `systray/src/`** (movido de `src/`): `systray.{h,cpp}` (host + watcher SNI, ahora con el
  probe del objeto y `becomeWatcher()`), `systraymodel.{h,cpp}` (ahora sobre `SystrayConfig`, no
  `DockConfig`), `systrayimageprovider.{h,cpp}`, `dbusmenu.{h,cpp}`, y `qml/SystrayMenu.qml`.
- **Nuevo en `systray/src/`**: `main.cpp` (candado de instancia única, `--show/--toggle/--hide/
  --settings/--screen/--dump`; construye el `SystrayHost` **siempre**, aun con `--hide`),
  `systraywindow.{h,cpp}` (superficie layer-shell modelada en `CmWindow`: anclada al borde del
  dock, `exclusiveZone=0`, se oculta al perder foco/Esc, `setMenuOpen()` frena el auto-hide
  mientras un menú está abierto), `systrayservice.{h,cpp}` (D-Bus `org.kdock.Systray` en
  `/Systray`), `systrayconfig.{h,cpp}` (`systray.conf`: tamaño/borde/alineación de ventana,
  tamaño y separación de íconos, opacidad, `closeOnFocusLoss`, `hiddenItems`, `preload`) y
  `systraysettingsdialog.{h,cpp}`. QML: `qml/Systray.qml` (la tira de íconos, extraída del viejo
  `systrayComp` del dock).
- **En kdock**: `src/systraylauncher.{h,cpp}`, el botón‑lanzador (token `systray`, componente
  `systrayButtonComp` en `Dock.qml`, context property `systrayLauncher`), y **nada más** — se
  quitaron `SystrayHost`/`SystrayModel`/`SystrayImageProvider`/`DBusMenu` del build de kdock, la
  maquinaria de exclusividad de `DockManager`, y `config.showSystray`/`systrayIconScale`/
  `systrayHiddenItems` de `DockConfig`. El token `systray` se conserva en `knownWidgetTokens()`.

**RESIDENTE, no como weather.** Un host SNI tiene que seguir corriendo toda la sesión para
juntar los ítems a medida que las apps se registran, así que `kdock-systray` **no termina al
cerrar la ventana** (solo la esconde) y kdock lo arranca al inicio con `--hide`
(`SystrayLauncher::startIfPreloading()`, `preload` default **true**). Por eso *Dock → Reiniciar*
**no** lo baja: matar el watcher dejaría a toda la sesión sin bandeja. Su `.desktop` no pide
privilegios (layer-shell no está restringido, SNI es D-Bus de sesión) → sin refresco de ksycoca.

Arnés y el detalle del watcher zombi de kded6: `CLAUDE.md` → *Arnés de `kdock-systray`*.
...
9b. **Submenu icons (`SubMenuDelegate.qml`, 2026-08-08)** — Every leaf of the dock's menus is an `IconMenuItem`, but the row that *opens* a submenu is not declared by us: Qt builds it from the parent menu's `delegate`, which by default is the stock `MenuItem` (no icon), so those rows were the only bare ones. Both menus that have submenus (`sectionMenu` and `contextMenu` in `Dock.qml`) now set `delegate: SubMenuDelegate {}`, and each submenu carries its icon name in a `property string menuIcon` that the delegate reads through its own `subMenu` property: `virtual-desktops` (Escritorio), `transform-move` (Ubicación), `application-menu` (Dock), `color-management` (`BackgroundColorMenu`), `contrast` (`ModeMenu`), `view-list-details` (`IconLabelMenu`), `view-list-tree` (`WidgetLabelMenu`). Three things that bite:
    ...
    **Submenu width (2026-08-12)**: a `Menu` sizes itself to its `implicitWidth`, which for these submenus is the **background floor** (200 px, `Basic/Menu.qml`), not the widest row — so "Nombre sobre el ícono" / "Nombre a la izquierda" / "Predeterminado (tema)" lost their last letters to the popup border at a larger font (measured: the widest row needs ~200 px of content, the menu gave it 186, and the `clip: true` ListView chopped the rest; the `Text` itself never elides because it takes its implicit width). The parent menus (`sectionMenu`/`contextMenu`/`dockMenu`/`desktopMenu`) already compensate with `width: Math.max(implicitWidth + 64, …)`; the three submenus with long labels — `IconLabelMenu`, `WidgetLabelMenu`, `BackgroundColorMenu` — now carry the same line. `ModeMenu`/`Ubicación` do not: their strings are short enough at any font. If a menu row ever clips again, this is the first thing to check.
10. **Brightness widget** — Drives **exactly one monitor**, the same way the volume widget drives only the default sink: wheel = ±5 % on that monitor, left click = 100 %, **right click = the *VideoEnergía* tab** (`DockWindow::openVideoSettings()`), which is where every other screen and the power profile live. Shift+right-click still opens the section menu.
    - **Which monitor**: `BrightnessControl::wheelDisplay()` resolves it fresh on every call from `ScreenBrightness` (PowerDevil, `org.kde.ScreenBrightness`) — `Brightness/wheelDisplay` empty means *auto*, `internal` pins it to the `brightnessctl` backlight, and anything else is a monitor **label**. Labels and not the D-Bus object names, which PowerDevil renumbers when a monitor sleeps.
    - **El orden de *auto* es: display interno de PowerDevil → el backlight de `brightnessctl` → el primer display.** El backlight va **antes** que el primer externo, y esa es la parte que no es obvia: que PowerDevil no reporte ningún display interno no quiere decir que no haya pantalla interna — con dock station lista los DDC y deja el panel del portátil al backlight. Con el orden ingenuo ("el primero"), en un portátil con monitor externo la rueda terminaba en el monitor de afuera, que es lo contrario de lo que hacen las teclas de brillo de la máquina (reportado 2026-08-12: *"por default selecciona el primer monitor, ¿está invertido?"*).
...
12. **Systray DBus (SNI)** — lives in the **`kdock-systray` binary now** (`systray/src/systray.cpp`), not in kdock; see *System tray* below for the split. `SystrayHost` implements the StatusNotifierItem protocol using the **`org.kde.*`** bus names, which is what the ecosystem actually implements (KDE, libappindicator, Qt/GTK trays); the `org.freedesktop.*` names are only registered as extra aliases when it has to *become* the watcher (bare wlroots, no existing watcher, or a dead one). Key points:
    - **Watcher discovery** (`ensureWatcher()`): if no watcher name is taken, become one ourselves. If one *is* taken, **do not trust the name — probe the object first** with an async `Get` of `RegisteredStatusNotifierItems` on `/StatusNotifierWatcher`. Only on a live reply do we adopt it as a host (`setupAsHost()`); on an error we `becomeWatcher()`. This is the 2026-08-24 fix for the **kded6 zombie**: on the mixed KF6 session (LXQt + `/opt/kde`) Ubuntu's kded6 holds `org.kde.StatusNotifierWatcher` but serves no object, which used to leave us the FDO-only watcher with 0 items while the real tray clients registered into kded6's dead one (`count == 0`, widget hidden). `becomeWatcher()` grabs both names (FDO is free; KDE cannot be stolen from kded6 at runtime — it never asked for `AllowReplacement`, so the real fix is to start early enough to win the name, or to free it by disabling kded6's broken module). The chosen name is `m_watcherService`.
    ...
    - **`RegisteredStatusNotifierItems`, `IsStatusNotifierHostRegistered` y `ProtocolVersion` son *propiedades*** (`as`, `b`, `i`), no métodos — en el watcher ajeno y en el nuestro. No es pedantería: `QDBusTrayIcon` de Qt (o sea **toda app Qt con `QSystemTrayIcon`**) lee `IsStatusNotifierHostRegistered` como propiedad, y si esa lectura falla se va al XEmbed de X11, que bajo Wayland significa **ningún ícono**. Verificado con un cliente `QSystemTrayIcon` de verdad: `isSystemTrayAvailable: true` y el ítem aparece en la lista.
        - **Fallback si el watcher muere entre el `isServiceRegistered` y el `RegisterStatusNotifierHost`** (2026-08-21): `ensureWatcher()` veía `kde:true` y usaba ese watcher, pero para cuando hacía el `asyncCall` el servicio ya no tenía owner y devolvía `No such object path '/StatusNotifierWatcher'`; antes se quedaba con `m_active=false` para siempre y la bandeja quedaba vacía aunque `showSystray=true` (reportado tras un flickeo intenso y varios reinicios). Ahora ese error hace fallback a convertirse en watcher nosotros mismos (`SniWatcherKdeAdaptor`/`SniWatcherFdoAdaptor`), igual que el caso `kde:false fdo:false`.
    ...
    - **Pixmap fallback**: items without a themed `IconName` expose only `IconPixmap` (`a(iiay)` = array of `(w, h, ARGB32-big-endian bytes)`). **These wire types MUST be registered with `qDBusRegisterMetaType` (KDbusImageStruct/KDbusImageVector, done in the SystrayHost ctor)** — otherwise `QDBusInterface::property()` returns empty and manual `QDBusArgument` demarshalling desyncs and **libdbus aborts** (`type struct not a basic type`). `SystrayItem::readPixmapProperty()` reads the property into `KDbusImageVector`, picks the largest frame and converts big-endian ARGB32 → native (`qFromBigEndian`). `SystrayImageProvider` (`image://systray/<service>`) serves it; on no icon it returns a transparent 1×1 (renders nothing, no QML warning). `SystrayModel` adds an `iconSerial` role (bumped every `readProperties()`) so the QML `Image` busts its cache on change. `Systray.qml` (in the binary) uses `image://icon/<name>` when `iconName` is set, else `image://systray/<service>@<iconSerial>`. Both image providers are registered on the `SystrayWindow` engine.
    - **Memoria de la vista** (2026-08-27): el `ToolTip` de cada ítem usa las propiedades adjuntas (`ToolTip.text/visible/delay`), no un `ToolTip { popupType: Popup.Window }` propio; la API adjunta comparte una sola instancia por `QQuickWindow`. El `SystrayMenu` de cada ítem vive detrás de un `Loader { active: false }`: se crea al llegar `menuReady()` y, una vez cerrado, un `Timer` de 30 s lo destruye (`active=false`), porque cerrar un `Popup.Window` no libera su `QQuickWindow`/render context. El `Image` de los pixmaps SNI lleva `cache: false` solo cuando `iconName` está vacío, y `IconMenuItem` hace lo mismo para `icon-data`; así el `iconSerial` puede seguir invalidando el frame sin retener todos los blobs históricos en la caché de Qt Quick. No volver a convertir esos popups en objetos declarativos por ítem: cada hover/clic añade superficies y memoria residente.
    - **Instancia única**: `alreadyRunning()` es solo una optimización; la comprobación autoritativa es `SystrayService::registerOnBus()`. `systray/src/main.cpp` debe salir inmediatamente si esa llamada devuelve `false`: dos lanzamientos simultáneos pueden pasar el primer chequeo, y una instancia que perdió `org.kdock.Systray` no puede entrar en `app.exec()` porque conservaría su host SNI, escena QML y recursos de render sin ser direccionable.
    ...
    - **Settings**: a "Mostrar la bandeja del sistema" checkbox in Settings → Widgets toggles `config.showSystray` (per-dock). **Any** dock can host the tray, but only one at a time per `canCoexist()` group: when another colliding dock owns it the checkbox stays **enabled** and checking it prompts `QMessageBox::question` to **move** the tray — on `Yes` the owner is cleared and the current dock set (`src/settingsdialog.cpp:2177`), on `No` the check is reverted; the move is not reversible except by re-enabling from the other dock. A note still names the owner (`systrayDockIdFor()` + `dockLabel()`). A "Systray icon scale" spinbox (20–100%) sets `config.systrayIconScale`, an **independent** scale (mirrors `widgetIconScale`): the derived read-only `config.systrayIconSize` = `qMax(16, iconSize * systrayIconScale%)` drives the tray icon cell/pixmap in `Dock.qml`'s `systrayComp` (so tray icons resize separately from other widgets, and `setIconSize` re-emits `systrayIconSizeChanged` for live rescale).
    - **Tooltips y flicker (2026-08-21)**: `config.showTooltips` + cada `ToolTip { popupType: Popup.Window }` en `Dock.qml` (`secMouse`, `mouseArea` de apps, systray, etc.) creaba su propia ventana `xdg_popup`. En un dock horizontal arriba, el tooltip aparecía bajo el cursor y le robaba el hover al `HoverHandler` del dock (`dockHover.hovered` caía), `revealWanted` dejaba de cumplirse y el dock se escondía — el tooltip se cerraba, el hover volvía, reaparecía: flickeo. El menú contextual (`Popup.Window` también) sufría lo mismo y desaparecía en ms. Fix: `root.tooltipVisible` agregado a `revealWanted` (como `appPreview.visible`) y cada `ToolTip` con `onVisibleChanged: root.tooltipVisible = visible` + `&& !root.menuOpen && !appPreview.visible` en su `visible`, `delay: 600` donde hacía falta, para que tooltip y menú/preview no compitan.
...
13. **Section layout & separators (`config.widgetOrder`)** — The dock content is an **ordered list of sections** stored in `DockConfig::widgetOrder` (a persisted `QStringList` of tokens). Known widget tokens: `menu`, `apps`, `clipboard`, `disks`, `network`, `volume`, `brightness`, `battery`, `clock`, `clock2`, `overview`, `movetodesktop`, `movetoscreen`, `maxmin`, `closewindow`, `nextwallpaper`, `darkmode`, `pager`, `autohide`, `showdesktop`, `systray`, `relanzadores`, `scriptrunners`, `session`, `settings`. On top of those come the **repeatable** tokens (`DockConfig::isRepeatableToken()`): `spring`, the **dynamic separator**; `sep`, the **static separator** (a fixed gap of `separatorSize` px, drawn by `staticSepComp`, cross-axis size = one widget icon so it never adds thickness); and the numbered families `gap<n>` (**transparent separator**, one width setting per instance — see its own section) and `appsel<n>`. The numbered ones are dispatched by a prefix helper in `Dock.qml` (`isGapToken()`, `isAppsWidget()`) instead of a `case`, which is what `tests/static/check-widget-tokens.py` knows to look for. `Dock.qml` renders them with a single-line `GridLayout` (`flow`/`rows`/`columns` switch by orientation) fed by a `Repeater` over `config.widgetOrder`, mapping each token to a `Component` via `componentFor()`.
...
19. **Presets: whole-configuration snapshots (Settings → Presets tab)** — replaced the old *Backup* tab. `ConfigArchive` (`src/configarchive.{h,cpp}`) reads and writes the configuration as a version-2 `.zip`. `configPrefixes()` is the single inventory for both export globs and accepted import entries: `kdock`, `previews`, `tilemenu`, `controlmanager`, `weather`, `desktop`, `systray` and `clipboard`, all as `<prefix>*.conf` in `ConfigArchive::configDir()` (the directory of `DockConfig::settingsFilePath()`). That covers the shared `kdock.conf` (enabledScreens, relanzadores, **script runners incl. their scripts**, favorites, audio and clipboard-capture preferences), per-dock files, and every current accessory configuration. It also exports direct user translation files as `translations/*.md` and adds `translations/.kdock-archive` as their replacement marker. Clipboard history and `clipboard-images/` are intentionally not archived: they are private content, not settings. Uses **QtCore's private `QZipWriter`/`QZipReader`** (`#include <private/qzipwriter_p.h>`; links `Qt6::CorePrivate`) — no external `zip`/`unzip` runtime dependency. Export starts with `DockConfig::syncAll()` and refuses an unreadable `kdock.conf`, preventing a stale or partial snapshot. Import validates `kdock.conf`, rejects config names containing either slash and accepts translation entries only when they are direct `.md` children (**anti zip-slip**), **backs up** current recognized configuration and translations to `backup-<timestamp>/`, then cleanly replaces only configuration families present in the archive. Old archives without the translation marker leave existing translations untouched.

    A **preset** is one of those archives kept in `<configDir>/presets/<name>.zip` — same format on purpose, so an exported `.zip` can be imported as a preset and back. `ConfigArchive::presetsDir()/presetNames()/presetPath()/savePreset()/deletePreset()/renamePreset()/importPreset()/sanitizePresetName()` are the whole API; `savePreset()` **removes the file before writing** (QZipWriter *appends* to an existing archive, so overwriting without that leaves two copies of every `.conf` inside and the import restores the older one). `sanitizePresetName()` is what keeps a user-typed name from becoming a path.

    UI (`SettingsDialog::createPresetsTab()`, `src/settingsdialog.cpp`): a combo whose first entry is **"Current"** (the config in use — an empty `userData`, and the one selection that disables everything else) followed by the saved presets; *Save…* / *Overwrite* / *Rename…* / *Delete*; *Apply* with an **"Apply without warning"** checkbox next to it (`DockConfig::presetApplyNoPrompt()`, shared key `presetApplyNoPrompt`); and at the foot of the tab *Export…* (current config → any `.zip`) and *Import…* (a `.zip` → **the preset list**, applying nothing). Without the checkbox, *Apply* first shows a warning naming the restart, with a **"Save first…"** button that saves the config being replaced as a preset (cancelling that cancels the apply).

    **Applying never writes the `.conf` files from the running process.** `SettingsDialog::applySelectedPreset()` calls `DockConfig::syncAll()` (flush the batched `QSettings` writes) and then `kdock::restartAll({"--apply-preset", <zip>})`; the **relaunched** process does the `importFrom()` in `main()` *before the first config read* (`src/main.cpp`, right before `DockConfig::qtStyle()`). Doing it in the outgoing process would let a live `QSettings` flush its dirty keys from its destructor **on top of** the config just installed. `restartAll()` drops any `--apply-preset` pair from the inherited command line — carrying it along would re-apply that preset on every later *Dock → Reiniciar*, silently throwing away everything changed since. Accessories are shut down first by `restartAll()`, so they flush before the new config lands and come back reading it.
...
+ the `focusRetry` timer, the same keyboard dance as the other dock popups — the password field needs key
...
| `DockConfig` | `config.*` | Exposed as context property; many `Q_PROPERTY` values. Section layout: `widgetOrder` (QStringList), `moveSection(from,to)`, `insertSpring(at)`, `insertSeparator(at)` (static `sep` token), **`insertGap(at)`** (devuelve el token `gap<n>`), `removeSectionAt(at)`, **`gapTokens()`**, **`gapFixedWidth(token)`/`setGapFixedWidth(token,on)`**, **`gapSize(token)`/`setGapSize(token,px)`** + read-only **`gapRevision`** (los dos getters son invocables: la binding toca la revisión primero, ver *Separador transparente*), **`insertAppsWidget(at)`** (devuelve el token `appsel<n>`), **`appsWidgetTokens()`**, **`widgetApps(token)`/`setWidgetApps(token,ids)`**, **`widgetOnlyPinned(token)`/`setWidgetOnlyPinned(token,on)`**, **`widgetExcludeOthers(token)`/`setWidgetExcludeOthers(token,on)`** (saltear lo que otro appsel ya dibuja), **`widgetUngroupWindows(token)`/`setWidgetUngroupWindows(token,on)`** (un ícono por ventana en ese widget), **`widgetAppPreview(token)`/`setWidgetAppPreview(token,on)`** + señal `widgetAppPreviewChanged(token)` (vista previa al hoverear, **por widget e independiente** de `appPreview`), **`reloadAppsWidget(token)`** (recarga manual: relee las listas del monitor y reconstruye ese widget), **`drawsAppCells()`**, `separatorSize` (px, both kinds of static separator). Dock length: `dockLength` (int 0-100, 0=auto, >0=% of screen edge). App-icon labels: `iconLabelMode` (0=icon only/default, 1=name below, 2=name at right, 3=name only, 4=name above, 5=name at left), `iconLabelWidth` (a **cap**), `iconLabelFontSize` (0=auto) + read-only `iconLabelFontPx`/`iconLabelLineHeight`/`iconLabelGap`/`dockThickness`/`effectiveLabelWidth`, and `setMeasuredLabelWidth(px)` (QML reports the widest name it draws; see App-icon labels). Section labels: `widgetLabelMode` (same values, no 3), `widgetName(token)`, `setWidgetName(token,name)` + read-only `widgetNamesRevision`. Auto-shrink: `autoShrinkIcons` (bool), `autoShrinkMinIconSize` (px). Ocultamiento: `hideMode` (0 siempre visible / 1 auto-hide / 2 intelligent hide / 3 las ventanas pasan abajo) + `autohide` (atajo de dos estados sobre él), read-only `edgeInset` (px de margen que la superficie paga como banda transparente propia en vez de como margen del ancla; 0 fuera de los modos que ocultan — es lo que `root.surfaceThickness`/`slider.revealedOffset` dibujan) y `hideAnimationMs`/`hideDelayMs` (compartidas, notifican con `hideTimingChanged`) — ver *Modos de ocultamiento*. Negritas: `labelBold` (bool, todos los nombres del dock). Renglones: `labelLines` (1 o 2 — con 2 los nombres se envuelven en vez de elidirse) + read-only `iconLabelBoxHeight`. Modo oscuro: `darkModeActive` (bool **resuelto**, read-only — el que QML tiene que leer), `darkMode` (flag propio del dock, solo vale cuando el alcance es por dock), `darkAccent`/`darkBackground` (QColor, app-wide), `setDarkModeActive(on)`, `showDarkMode` — todas notifican con `darkModeChanged`. Brillo neón (contorno luminoso del panel, global opt-in por monitor, estáticos en `kdock.conf`): `neonActive` (bool **resuelto** para el dock, read-only — el que QML lee), `neonColor`/`neonIntensity`/`neonSize` (read-only, passthrough de los estáticos `neonColorSetting()`/`neonIntensitySetting()`/`neonSizeSetting()`), `neonGlowOuter`/`neonGlowMargin`/`neonEdgeGlow` (read-only — el estilo halo exterior resuelto para el dock y cuánto crece la superficie por lado; ver *Brillo neón*) — notifican con `neonChanged`; estáticos `neonEnabled()/setNeonEnabled()`, `neonScreens()/neonScreenEnabled()/setNeonScreenEnabled()`, `setNeonColor()/setNeonIntensity()/setNeonSize()`, `neonGlowMode()/setNeonGlowMode()` (0 rim / 1 halo exterior), `neonWithDarkMode()/setNeonWithDarkMode()` (link con modo oscuro, casilla en DarkMode — `neonActive` lo hace un OR con el maestro), `notifyNeonChanged()`. QML empuja `dockWindow.setContentRects()` (input region del halo). Clic derecho (submenú `NeonMenu.qml`): read-only `neonEnabledGlobal`/`neonPresetColors` (5 hexes de `neonQuickColors()`), per-dock `neonDockDisabled` (READ/WRITE, gana sobre todo en `neonActive()`), invocables `setNeonColorGlobal()`/`setNeonEnabledGlobal()` — ver *Brillo neón*. ColorAuto: `autoColorActive` (bool), `autoBackground`/`autoAccent` (QColor) — read-only y **no persistidas**, las empuja `AutoColorScheme` con `setAutoColors()` una vez por cambio de fondo y notifican con `autoColorChanged`; son un override de lectura como los del modo oscuro, ver *ColorAuto*. Qt style: `qtStyle` (QString, app-wide via static, persisted in `kdock.conf`) — el widget style de Qt para todo el proceso: `Breeze`, `Fusion`, `Oxygen`, `Windows`…; vacío (default) = Fusion. La UI es un combo en General (lista `QStyleFactory::keys()`) y el cambio requiere reiniciar kdock para surtir efecto. El valor se aplica a widgets (`app.setStyle`) **y** a los menús QML del dock (`QQuickStyle::setStyle`). Como los estilos de widgets y los de Qt Quick Controls 2 son conjuntos distintos, una lambda `qqc2Style()` mapea: vacío/Breeze/Oxygen/etc. → `Fusion`; solo `Basic`/`Fusion`/`Material`/`Universal`/`Imagine` pasan tal cual. El cambio del `import QtQuick.Controls.Basic` al import sin sufijo en los ~40 `.qml` fue necesario para que `setStyle` surta efecto (ver commit 6694ec0). Vista previa al hoverear (compartidas, notifican con `appPreviewChanged`): `appPreview` (bool, **default false**, manda **solo sobre el bloque Apps** — cada widget appsel tiene su `widgetAppPreview(token)`), `appPreviewSize` (px de ancho, 120-600, default 260 — el alto sale de la proporción de la ventana; vale para los dos), `appPreviewDelayMs` (0-2000, default 250, ídem) — ver *Vista previa de ventana*. Íconos de apps: `showAppIcons` (bool, default true — en false el dock es una barra de solo widgets y la sección `apps` sale de `dockThickness()`). Pickers de apariencia: `showIconThemes`, `showColorSchemes`. Widget Control Manager: `showControlManager`, `controlManagerIcon`, `controlManagerDisplay` (0 solo ícono / 1 ícono+texto / 2 solo texto), `controlManagerText` (vacío = el reloj, con `controlManagerFormat`), `controlManagerFontSize` (0 = automático: la fuente del reloj y, si no, una fracción del ícono; independiente de los modos de etiqueta — ver *Control Manager*). App menu (all in the shared-when-enabled menu group): `menuIcon`, `menuPopupWidth`/`menuPopupHeight`, `menuColumns` (1-8), `menuAppIconSize` (px, both list layouts), `menuGridSpacing` (px), `menuEditorApp` (.desktop id or command), `showMenuPower`, `menuFavorites` |
...
| `DockModel` | `dockModel` (model), y uno por widget *Apps Seleccionables* vía `dockWindow.appsModelFor(token)` | `activate(row)`, `launch(row)`, `togglePinned(row)`, `closeAll(row)`, `windowList(row)`, `moveItem(from, to)`, `activateWindow(row, idx)`, `syncWindows()`, `sendToDesktop(row, position)` (mueve **todas** las ventanas del ícono a un escritorio 1-based, sin seguirlas), **`previewWindow(row)`** (→ `{uuid, width, height}` de la ventana que se previsualiza al hoverear; **mapa vacío** si es separador, si no hay ventanas o si el backend no da uuid — ver *Vista previa de ventana*). Roles: `name`, `iconName`, `pinned` (con *Desagregar ventanas* lo devuelven **todas** las filas de una app de la lista, no solo la del lanzador — ver *Apps Seleccionables*), `windowCount`, `active`, `minimized`, `title`, `isSeparator`, `separatorTransparent` (el separador inline que ocupa lugar y no dibuja línea) |
| `DockWindow` | `dockWindow` | `windowsOverlap` (bool read-only: en modo *dodge*, una ventana llega al dock — ver *Modos de ocultamiento*), `setHidden(bool)`, `openSettings()`, `openSettingsToDock()`, `createEmptyDock()`, `deleteDock()`, `moveToNextMonitor()`, `copyToNextMonitor()`, `restart()`, `quit()`, **`appsModelFor(token)`** (el `DockModel` de un widget *Apps Seleccionables*; con token vacío devuelve el del bloque de apps) |
...
| `AppPreviews` | `appPreviews` (en kdock) | Miniaturas de ventana para las vistas previas del bloque de apps: `available()`, `request(uuid, maxWidth)`, `cancel(uuid)`, `revision(uuid)` (0 = todavía no hay captura), `thumbId(uuid)` (la forma url-safe: **nunca armar la url `image://` con el uuid crudo**) + señal `thumbnailReady(uuid, revision)`. Una instancia por proceso, en `DockManager::Shared`; sirve `image://thumb/<thumbId>@<rev>` en cada engine |
...
| `SystrayLauncher` | `systrayLauncher` | `toggle(screenName)`, `openSettings()`; static `preload()`/`setPreload()`/`startIfPreloading()`/`quitRunning()`/`installed()`/`running()`. Thin launcher for the `kdock-systray` binary (D-Bus `org.kdock.Systray`); the dock's `systray` widget button drives it. The tray itself (`SystrayModel`/`SystrayHost`/DBusMenu) lives in that binary, exposed to *its* QML as `systray` — see *System tray* |
...
| `ClipboardLauncher` | `clipboardLauncher` | `toggle(screenName, anchorX, anchorY, edge)`; asynchronous D-Bus launcher for the independent `kdock-clipboard` process |
...
- Additional widgets (battery, etc.). ✅ Done: overview, move-to-next-desktop, move-to-next-monitor, second clock (styled tooltip), next-wallpaper (per-monitor), script runner (`sh -c`), session/power menu, kdock-settings button, clipboard launcher (`kdock-clipboard`, see **Clipboard history**).
