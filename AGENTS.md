...
- **Nunca se muestra sin captura**: si no hay revisión todavía, la ventana queda oculta hasta
  ...
  autorizar degrada a *"no aparece nada"* en vez de a un marco vacío. Mientras está visible, un
  timer de 1 s vuelve a pedir la captura (probado: el preview de un video cambia de cuadro).
...
- **Per-panel background image (tiled)**: `config.panelImage` (absolute path, **per-dock**, empty = none) + read-only `panelImageUrl` (`QUrl::fromLocalFile`). `Dock.qml` draws an `Image` (`id: bgImage`) inside the `background` Rectangle, **over** the base color, `fillMode` = `TileHorizontally` (horizontal docks) / `TileVertically` (vertical docks) — scaled to the dock thickness, repeated along its length. Honors `config.opacity`. Rounded corners in non-panel mode via `layer.effect: OpacityMask` (needs `import Qt5Compat.GraphicalEffects` — its **only** remaining use since the neon glow moved to `MultiEffect` in 2026-08-26; runtime dep `qml6-module-qt5compat-graphicaleffects`, plus `qml6-module-qtquick-effects` for `MultiEffect`). Chosen via a `QFileDialog` button in Settings → **General → "Panel image"** (+ "Clear"). **Note**: the config stores only the *path*; a preset (and the exported `.zip`) therefore does **not** bundle the image file.
...
- **El rim** (default) se dibuja **por encima** del panel (`z: 3`, dentro del `Rectangle{id:background}`, uno por run): un `Rectangle` de solo borde (fuente `visible:false`, `layer.enabled`) blureado con un `MultiEffect` (`QtQuick.Effects`) para el resplandor que se derrama **hacia adentro**, más una línea nítida (`Qt.lighter(neonColor,1.4)`, 2 px) que marca el tubo. Existe porque el panel llenaba **toda** la superficie layer-shell, así que un glow hacia afuera lo recortaba el compositor — que es exactamente lo que el halo exterior resuelve.
- **El resplandor lo dibuja `MultiEffect`, no el `Glow` de `Qt5Compat.GraphicalEffects`** (migrado 2026-08-26): `blurEnabled + blur:1 + blurMax:round(neonSize)` sustituye al `radius`/`samples`/`spread` del `Glow`, y `autoPaddingEnabled` reemplaza a `transparentBorder` para que el blur del halo exterior se derrame más allá de la fuente. Un solo pase padded en vez del blur separable multi-pase del `Glow`, con **menos buffers intermedios por run** — importa porque hay **un FBO de glow por run y por dock**, y el neón es opt-in por monitor sobre docks que pueden ser muchos. La forma de la fuente y las líneas nítidas del tubo no cambian. `Qt5Compat.GraphicalEffects` sigue importado **solo** por el `OpacityMask` de la imagen de panel.
- **El halo exterior (2026-08-26) le da al glow lugar transparente para derramarse hacia afuera**, reusando la primitiva de los modos de ocultamiento (`edgeInset`/`surfaceThickness`): la **superficie crece más allá del dock** y el panel se **insetea** dentro de ese sobrante transparente, así el `MultiEffect` (dibujado **detrás** del panel, un `Item` por run que crece `glowMargin` por lado con la fuente en la forma del panel) se ve entero. Piezas:
...
- **El `MultiEffect` es un shader de GPU, así que el arnés de Xvfb NO lo muestra** (backend software = sin shaders, el PNG sale con las líneas nítidas del tubo pero sin el resplandor blureado — vale igual para el `Glow` viejo, por eso la migración a `MultiEffect` de 2026-08-26 se comprobó con build + qmllint + `qml.smoke` + geometría headless, y el resplandor en sí se mira en la sesión real). Headless se verifica la **geometría** (el tamaño de la superficie crece exacto: un dock centrado flotante 768×68 pasa a 804×104 con `neonSize=18`; `alignment`/panel/auto-hide dan los tamaños de la matriz), la lógica (`neonGlowOuter`/`neonGlowMargin`/`neonEdgeGlow` con una sonda que linkea `kdock_core`) y la persistencia; el resplandor en sí solo se ve en la sesión Wayland real con GPU (verificado 2026-08-26: el halo se derrama por los cuatro lados de un dock flotante, el rim no).
