...
- **El render es un rim, no un halo hacia afuera, y hay una razón dura.** El panel llena **toda** la superficie layer-shell (el `screenMargin` es un margen del compositor, `dockwindow.cpp:392`, no un inset dentro de la escena), así que un glow hacia afuera lo recorta el compositor. Por eso el efecto se dibuja **por encima** del panel (`z: 3`, dentro del `Rectangle{id:background}`, uno por run): un `Rectangle` de solo borde con `layer.effect: Glow` (de `Qt5Compat.GraphicalEffects`, ya importado) para el resplandor que se derrama hacia adentro, más una línea nítida (`Qt.lighter(neonColor,1.4)`, 2 px) que marca el tubo. El glow que se derrama hacia afuera se recorta, y está bien: el look elegido es el contorno pegado al borde.
- **El `Glow` es un shader de GPU, así que el arnés de Xvfb NO lo muestra** (backend software = sin shaders, el PNG sale sin el resplandor). Lo verificable headless es la lógica (`neonActive` resuelve bien, los valores llegan a QML, la línea nítida dibuja) y la persistencia (sonda de la solapa que linkea `kdock_core` y relee el `.conf`); el resplandor en sí solo se ve en la sesión Wayland real con GPU.
- **UI (solapa)**: solapa **Neon** (`SettingsDialog::createNeonTab()`, junto a DarkMode) con el checkbox maestro *Activar Neon*, el selector de color (`makeColorButton` con `neonColorSetting`/`setNeonColor`), dos `QSlider` (intensidad 0-100→0..1, tamaño 0-40 px) y el `QListWidget` de monitores calcado del de Desktop (filas checkables sembradas de `neonScreenEnabled()`, greyed cuando el maestro está apagado). Sin refresco de ksycoca ni checklist de siete archivos: es config-only + render.
