...
- **Los docks se pintan con un override de lectura, igual que el modo oscuro** — y por la misma razón: el `panelColor` del usuario queda intacto en su `.conf` y apagar la feature restaura por construcción, sin snapshot que se pueda desincronizar. `DockConfig` suma `autoColorActive` / `autoBackground` / `autoAccent`, **no persistidas** (`setAutoColors()` no toca `QSettings`), y `Dock.qml` las lee en `dockBaseColor`, `dockTextColor` y `widgetIconTheme`. El modo oscuro gana la precedencia, que es gratis porque ColorAuto se apaga solo mientras aquel está puesto.
...
- **Tres puntos de entrada para generar a mano**, todos contra el mismo `generateNow()`:
  - **La solapa**: los botones *Generar Color* y *Guardar Color Scheme*, deliberadamente **fuera de `m_colorAutoBody`** —el `QWidget` que el interruptor maestro deshabilita— porque tienen que andar con ColorAuto apagado. *Aplicar ahora* sí queda adentro: es el "releer el fondo" del modo automático, otra cosa.
