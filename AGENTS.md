...
- **Los cuatro triggers** (todos desembocan en `refresh()`, con un `QTimer` de 1200 ms que coalesce): el widget/atajo de *siguiente fondo* (`WallpaperControl::wallpaperAdvanced`), los wallpapers por escritorio virtual (`DesktopWallpapers::wallpapersApplied`), el cambio de escritorio virtual, y el botón *Aplicar ahora*. **No hay watcher ni polling**: un cambio hecho fuera de kdock (el pase de diapositivas de KDE avanzando solo, Preferencias del Sistema) no se detecta, por decisión de diseño.
...
- **No es un widget del dock**, así que de los siete archivos de la convención de `CLAUDE.md` no toca `knownWidgetTokens()`, ni un `Component` en `Dock.qml`, ni `dockwindow.cpp`. Sí toca `Dock.qml`, pero por los tres bindings de color, no por una sección.
