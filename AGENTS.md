...
- `setDarkModeActive(bool)` (`Q_INVOKABLE`) es lo que llaman el submenú y el widget: escribe **donde está el valor efectivo** (la lista de excepciones cuando el interruptor global está prendido, el flag propio si no), así el clic nunca queda anulado en silencio por el interruptor global.
