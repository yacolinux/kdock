...
- **Dónde vive el estado.** Por dock, en su `.conf`: `darkMode` (bool) y `showDarkMode` (el widget). App-wide, en el `kdock.conf` compartido: `darkModeAllDocks` (el checkbox único), `darkModeExceptions` (lista de dockIds), `darkAccent` y `darkBackground` (los dos colores son **únicos para toda la app**, no por dock).
- **La resolución es una sola función**, `DockConfig::darkModeActive()`: si `darkModeAllDocks()` está prendido gana él y decide la lista de excepciones; si no, manda el flag propio del dock. Resolver en vez de propagar es lo que hace que una excepción **no necesite ninguna escritura** en el `.conf` del dock exceptuado — incluidos los docks que no están corriendo.
- **Una sola señal para todo el grupo**, `darkModeChanged()`. Los cuatro setters estáticos terminan en `notifyDarkModeChanged()`, que la emite sobre **cada** `DockConfig` de `s_instances`: el interruptor global, las excepciones y los dos colores son de proceso, así que un cambio tiene que repintar los 15 docks, no solo el que se está editando.
- **El interruptor app-wide define el *alcance* de la acción, no un default.** `setDarkModeActive(bool)` (`Q_INVOKABLE`, lo que llaman el submenú, el widget y el checkbox de la solapa) mira `darkModeAllDocks()`: si está prendido, elegir un modo desde **cualquier** dock vale para **todos** — limpia la lista de excepciones y, al apagar, baja además el flag propio de cada `DockConfig` vivo antes de bajar el interruptor (si no, cada dock vuelve a su propio flag y el que lo tuviera en true se quedaría oscuro). Si está apagado, escribe solo el flag del dock.
  - **Bug 2026-08-02**: antes, apagar desde el submenú con el interruptor prendido **agregaba una excepción** en vez de apagar. El dock quedaba normal pero el interruptor seguía prendido, y el checkbox de la solapa —que mostraba el flag propio del dock, no el efectivo— seguía diciendo "activo". Dos errores que se tapaban: la semántica del alcance y un control que leía la variable equivocada.
  - Las **excepciones son una herramienta de edición de la solapa**, no algo que el submenú produzca: se tildan a mano en la lista y cualquier cambio de modo las limpia.
- **Los tres controles son un solo interruptor con tres caras.** El checkbox de la solapa lee `darkModeActive()` (el valor efectivo) y escribe con `setDarkModeActive()`, igual que el submenú y el widget; su etiqueta sigue el alcance ("Activado en este dock" / "Activado en todos los docks"). La solapa se redibuja entera desde la config —no desde los otros controles— en un único `syncAll()` colgado de `darkModeChanged`, con `QSignalBlocker` sobre los tres widgets porque la lista de excepciones es justamente lo que `setDarkModeExceptions()` cambia.
